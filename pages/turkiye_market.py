...
# Türkiye market page for all-results (parametric) pilot
# Note: This page is registered in dash_app.py, not here

# Reuse the Turkiye explorer dataset for plotting
from pages.turkiye_explorer import df as df_plot  # already has proper column mappings

# Real-world market scenarios for Türkiye (kNN search dataset)
...
def find_closest_instances(selected_idx, n_neighbors=5):
    """Find closest instances using normalized heating/cooling between df_plot and df_search."""
    ...
    # Figure out which columns in df_search correspond to predicted loads
    ...
    if 'predicted_heating_load' not in df_search.columns or 'predicted_cooling_load' not in df_search.columns:
        ...
        if heating_cols and cooling_cols:
            ...
            print("WARNING (TUR market): could not find heating/cooling cols in df_search")
            return []

    feature_matrix = df_search[search_features].values

    # Normalize both datasets independently then compare
...
layout = dbc.Container([
    dbc.Row([
        dbc.Col([
            html.H1(
                "Closest Market-Available Scenarios – Türkiye (All Solutions)",
                style={
    ...
    dbc.Row([
        dbc.Col([
            html.Div([
                ...
                dcc.Slider(
                    id="tur-market-neighbors-slider",
                    min=1,
                    max=10,
                    step=1,
                    value=5,
    ...
    dbc.Row([
        dbc.Col([
            dbc.Card(id="tur-market-instance-table-card", children=[
                dbc.CardHeader([
                ...
                dbc.CardBody([
                    ...
                    dbc.Row([
                        dbc.Col([
                            html.Div(id="tur-market-table-info", style={
                                "fontSize": "12px",
                                "color": "#6C757D",
                                "fontWeight": "normal",
                                "padding": "8px 0",
    ...
    dbc.Row([
        dbc.Col([
            dbc.Button(
                "← Back to Türkiye All Solutions",
                href="/pilot_tur_explorer",
                external_link=True,
                color="primary",
                style={
                    "marginTop": "20px",
    ...
    dcc.Store(id="tur-market-selected-point-store", data=None),
    dcc.Store(id="tur-market-instances-store", data=[]),
    dcc.Store(id="tur-market-table-selected-instances", data=[]),
    dcc.Download(id="tur-market-download-instances"),
    dcc.Store(id="tur-market-layers-data", data={}),
    dcc.Store(id="tur-market-hover-state", data={}),

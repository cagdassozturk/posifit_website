...
import threading
import re
from pages.turkiye_optimized import df
from dash import callback, Output, Input, ctx, html, dcc, State, clientside_callback, callback_context

# ==========================================
# PERFORMANCE OPTIMIZATIONS - TURKIYE OPT
# ==========================================

_cached_data_turkiye_opt = None
_cached_df_with_id_turkiye_opt = None

def get_cached_data_turkiye_opt():
    """Load Turkiye optimized data once and cache it"""
    global _cached_data_turkiye_opt, _cached_df_with_id_turkiye_opt
    if _cached_data_turkiye_opt is None:
        _cached_data_turkiye_opt = pd.read_csv("data/turkiye/TUR_pareto_optimized.csv")
        ...
        _cached_data_turkiye_opt = _cached_data_turkiye_opt.rename(columns=column_mapping)
        
        # Add co2 column if missing
        if 'co2' not in _cached_data_turkiye_opt.columns:
            _cached_data_turkiye_opt['co2'] = 0
        
        _cached_df_with_id_turkiye_opt = _cached_data_turkiye_opt.copy()
        _cached_df_with_id_turkiye_opt['ID'] = _cached_df_with_id_turkiye_opt.index + 1
    return _cached_data_turkiye_opt, _cached_df_with_id_turkiye_opt

def get_selectable_data_turkiye_opt():
    """Get selectable data for optimized page"""
    df, df_with_id = get_cached_data_turkiye_opt()
...
# ==========================================
# CACHE DICTIONARIES - TURKIYE OPT
# ==========================================

_dropdown_cache_turkiye_opt = {}
_param_cache_turkiye_opt = {}
_comparison_cache_turkiye_opt = {}
_graph_cache_turkiye_opt = {}

def format_co2_value_turkiye_opt(co2_value):
    """Format CO2 value"""
    try:
...
# ==========================================
# VIRTUAL CONTENT UPDATE - TURKIYE OPT
# ==========================================

@callback(
    [Output('tur-opt-virtual-scroll-container', 'children'),
     Output('tur-opt-virtual-scroll-position', 'data')],
    [Input('tur-opt-virtual-scroll-container', 'scrollTop'),
     Input('tur-opt-comparison-selected-instances-store', 'data'),
     Input('tur-opt-sort-by-dropdown', 'value'),
     Input('tur-opt-sort-order-radio', 'value'),
     Input('tur-opt-virtual-scroll-position', 'data')],
    [State('tur-opt-virtual-item-height', 'data'),
     State('tur-opt-virtual-container-height', 'data')],
    prevent_initial_call=False
)
def update_virtual_content_combined_turkiye_opt(scroll_top_prop, selected_instances, sort_by, sort_order, scroll_position_data, item_height, container_height):
    """Virtual scrolling for Turkiye optimized"""
    ...
    try:
        ...
        df, df_with_id = get_cached_data_turkiye_opt()
        selectable_df = get_selectable_data_turkiye_opt()
        ...
        cache_key = f"tur_opt_{scroll_group}_{selected_key}_{sort_by}_{sort_order}"
        
        triggered_id = ctx.triggered_id
        
        if cache_key in _dropdown_cache_turkiye_opt and triggered_id != 'tur-opt-selected-instances-store':
            return _dropdown_cache_turkiye_opt[cache_key], scroll_top
        ...
        for i in range(visible_start, visible_end):
            ...
            unique_checkbox_id = f"tur-opt-virtual-checkbox-{instance_id}-{scroll_group}"
            ...
            visible_items.append(
                html.Div([
                    ...
                    html.Div([
                        dcc.Checklist(
                            id={"type": "tur-opt-virtual-checkbox", "index": instance_id, "unique": unique_checkbox_id},
                ...
                id=f"tur-opt-virtual-item-{instance_id}",
        ...
        if len(_dropdown_cache_turkiye_opt) > 50:
            old_keys = list(_dropdown_cache_turkiye_opt.keys())[:25]
            for key in old_keys:
                del _dropdown_cache_turkiye_opt[key]
        
        _dropdown_cache_turkiye_opt[cache_key] = visible_items
        
        return visible_items, scroll_top
        
    except Exception as e:
        print(f"TUR OPT Virtual content error: {e}")
        import traceback
        traceback.print_exc()
...
# ==========================================
# WARNING OVERLAY - TURKIYE OPT
# ==========================================

@callback(
    [Output('tur-opt-warning-overlay', 'children'),
     Output('tur-opt-warning-overlay', 'style')],
    Input('tur-opt-selection-warning-state', 'data'),
    [State('tur-opt-dropdown-open-state', 'data')],
    prevent_initial_call=False
)
def update_warning_overlay_turkiye_opt(warning_state, dropdown_is_open):
...
@callback(
    Output('tur-opt-warning-overlay', 'style', allow_duplicate=True),
    Input('tur-opt-dropdown-open-state', 'data'),
    [State('tur-opt-selection-warning-state', 'data'),
     State('tur-opt-warning-overlay', 'style')],
    prevent_initial_call=True
)
def update_warning_position_on_dropdown_toggle_turkiye_opt(dropdown_is_open, warning_state, current_style):
...
# ==========================================
# SELECTION HANDLING - TURKIYE OPT
# ==========================================

@callback(
    [Output('tur-opt-comparison-selected-instances-store', 'data'),
     Output('tur-opt-selection-warning-state', 'data')],
    [Input({"type": "tur-opt-virtual-checkbox", "index": dash.ALL, "unique": dash.ALL}, 'value'),
     Input('tur-opt-search-input', 'n_submit')],
    [State({"type": "tur-opt-virtual-checkbox", "index": dash.ALL, "unique": dash.ALL}, 'id'),
     State('tur-opt-comparison-selected-instances-store', 'data'),
     State('tur-opt-search-input', 'value')],
    prevent_initial_call=True
)
def handle_selection_and_search_with_warning_turkiye_opt(all_values, n_submit, all_ids, current_selected, search_value):
    """Selection handling for Turkiye optimized"""
    ...
    # Handle search input
    if triggered_id == 'tur-opt-search-input' and n_submit and search_value is not None:
        try:
            ...
            if 1 <= instance_id <= 100:
                ...
                if instance_id not in new_selected:
                    ...
                    print(f"TUR OPT SEARCH: Added S{instance_id}")
                
                def delayed_cache_clear():
                    time.sleep(0.1)
                    global _dropdown_cache_turkiye_opt
                    _dropdown_cache_turkiye_opt.clear()
    ...
    if all_values and all_ids:
        try:
            ...
            if 'tur-opt-virtual-checkbox' not in prop_id:
                raise PreventUpdate
            ...
            print(f"TUR OPT Selection error: {e}")
    
    return current_selected, warning_state


# ==========================================
# DROPDOWN STATE - TURKIYE OPT
# ==========================================

@callback(
    [Output('tur-opt-virtual-scroll-container', 'style'),
     Output('tur-opt-dropdown-open-state', 'data'),
     Output('tur-opt-virtual-dropdown-trigger', 'children')],
    [Input('tur-opt-virtual-dropdown-trigger', 'n_clicks'),
     Input('tur-opt-comparison-selected-instances-store', 'data')],
    [State('tur-opt-dropdown-open-state', 'data')],
    prevent_initial_call=True
)
def handle_dropdown_state_combined_turkiye_opt(n_clicks, selected_instances, is_open):
    """Dropdown toggle"""
    
    triggered_id = ctx.triggered_id
    
    if triggered_id == 'tur-opt-virtual-dropdown-trigger' and n_clicks:
        ...
        trigger_text = get_trigger_text_turkiye_opt(selected_instances)
        return style, new_state, trigger_text
    
    if triggered_id == 'tur-opt-selected-instances-store':
        current_style = {
...
def get_trigger_text_turkiye_opt(selected_instances):
...
# ==========================================
# SEARCH TOGGLE - TURKIYE OPT
# ==========================================

@callback(
    [Output('tur-opt-search-input-container', 'className'),
     Output('tur-opt-search-toggle-state', 'data'),
     Output('tur-opt-search-input', 'value'),
     Output('tur-opt-search-toggle-button', 'className')],
    [Input('tur-opt-search-toggle-button', 'n_clicks'),
     Input('tur-opt-search-input', 'n_submit')],
    [State('tur-opt-search-toggle-state', 'data'),
     State('tur-opt-search-input', 'value')],
    prevent_initial_call=True
)
def handle_search_toggle_combined_turkiye_opt(toggle_clicks, submit_count, is_open, search_value):
    """Handle search visibility"""
    
    triggered_id = ctx.triggered_id
    
    if triggered_id == 'tur-opt-search-toggle-button':
    ...
    if triggered_id == 'tur-opt-search-input' and search_value:
...
# ==========================================
# PARAMETER UPDATE - TURKIYE OPT
# ==========================================

@callback(
    Output('tur-opt-instance-parameters', 'children'),
    Input('tur-opt-comparison-selected-instances-store', 'data'),
    prevent_initial_call=False
)
def update_instance_parameters_optimized_turkiye_opt(selected_instances):
    """Parameter updates with caching"""
    
    df, _ = get_cached_data_turkiye_opt()
    ...
    cache_key = f"tur_opt_{selected_id}_baseline_{baseline_mode}"
    
    if cache_key in _param_cache_turkiye_opt:
        return _param_cache_turkiye_opt[cache_key]
    ...
    _param_cache_turkiye_opt[cache_key] = result
    return result


# ==========================================
# COMPARISON UPDATE - TURKIYE OPT
# ==========================================

@callback(
    Output('tur-opt-instance-comparison-content', 'children'),
    Input('tur-opt-comparison-selected-instances-store', 'data'),
    prevent_initial_call=False
)
def update_instance_comparison_optimized_turkiye_opt(selected_instances):
    ...
    def format_value(raw_val, metric_key=None):
        try:
            if metric_key == "co2":
                return format_co2_value_turkiye_opt(raw_val)
            else:
                if pd.isna(raw_val):
    ...
    def create_instance_content_with_remove_btn(instance_label, instance_id):
        ...
        return html.Div([
            html.Div([
                ...
                html.Button(
                    "×", 
                    id={"type": "tur-opt-remove-instance-btn", "index": instance_id},
...
# ==========================================
# REMOVE BUTTON - TURKIYE OPT
# ==========================================

@callback(
    Output('tur-opt-comparison-selected-instances-store', 'data', allow_duplicate=True),
    Input({"type": "tur-opt-remove-instance-btn", "index": dash.ALL}, 'n_clicks'),
    State('tur-opt-comparison-selected-instances-store', 'data'),
    prevent_initial_call=True
)
def handle_remove_instance_button_turkiye_opt(n_clicks_list, current_selected):
    ...
    global _dropdown_cache_turkiye_opt, _comparison_cache_turkiye_opt, _graph_cache_turkiye_opt
    _dropdown_cache_turkiye_opt.clear()
    _comparison_cache_turkiye_opt.clear()
    _graph_cache_turkiye_opt.clear()
    
    return new_selected


# ==========================================
# GRAPH UPDATE - TURKIYE OPT
# ==========================================

@callback(
    Output('tur-opt-comparison-graph', 'figure'),
    Input('tur-opt-comparison-selected-instances-store', 'data'),
    prevent_initial_call=False
)
def update_comparison_graph_optimized_turkiye_opt(selected_instances):
    """Graph updates"""
    ...
    cache_key = "tur_opt_" + "_".join(map(str, sorted(selected_instances)))
    
    if cache_key in _graph_cache_turkiye_opt:
        return _graph_cache_turkiye_opt[cache_key]
    ...
    _graph_cache_turkiye_opt[cache_key] = fig
    return fig


# ==========================================
# CLIENTSIDE CALLBACKS - TURKIYE OPT
# ==========================================
...
clientside_callback(
    ...
    function(n_clicks) {
        if (n_clicks > 0) {
            setTimeout(function() {
                const container = document.getElementById('tur-opt-virtual-scroll-container');
                        ...
                        scrollTimeout = setTimeout(function() {
                            if (Math.abs(scrollTop - lastScrollTop) > 3) {
                                ...
                                if (window.dash_clientside && window.dash_clientside.set_props) {
                                    try {
                                        window.dash_clientside.set_props('tur-opt-virtual-scroll-position', {
                                            data: scrollTop
                                        });
                                    } catch (error) {
                                        console.error('TUR OPT scroll error:', error);
                                    }
                                }
                            }
                        }, 30);
    ...
    """,
    Output('tur-opt-virtual-scroll-container', 'data-scroll-initialized'),
    Input('tur-opt-virtual-dropdown-trigger', 'n_clicks'),
    prevent_initial_call=True
)
...
clientside_callback(
        ...
        if (n_clicks > 0 && !window.turOptOutsideClickAttached) {
            setTimeout(function() {
                document.addEventListener('click', function(e) {
                    const dropdown = document.getElementById('tur-opt-virtual-dropdown-trigger');
                    const menu = document.getElementById('tur-opt-virtual-scroll-container');
                    const warning = document.getElementById('tur-opt-warning-overlay');
                    ...
                    if (dropdown && menu && 
                        ...
                        if (menu.style.display === 'block') {
                            ...
                            if (window.dash_clientside && window.dash_clientside.set_props) {
                                window.dash_clientside.set_props('tur-opt-dropdown-open-state', {data: false});
                            }
                        }
                    }
                }, { passive: true });
    ...
    """,
    Output('tur-opt-dummy-outside-click', 'children'),
    Input('tur-opt-virtual-dropdown-trigger', 'n_clicks'),
    prevent_initial_call=True
)

@callback(
    [Output('tur-opt-virtual-item-height', 'data'),
     Output('tur-opt-virtual-container-height', 'data')],
    Input('tur-opt-virtual-dropdown-trigger', 'n_clicks'),
    prevent_initial_call=False
)
def validate_virtual_dimensions_turkiye_opt(n_clicks):
    """Validate dimensions"""
    return 40, 320


@callback(
    Output('tur-opt-virtual-scroll-container', 'data-total-items'),
    Input('tur-opt-sort-by-dropdown', 'value'),
    prevent_initial_call=False
)
def validate_total_items_turkiye_opt(sort_by):
...
# ==========================================
# TABLE CALLBACKS - TURKIYE OPT
# ==========================================

@callback(
    [Output("tur-opt-instance-table", "data"),
     Output("tur-opt-table-info", "children"),
     Output("tur-opt-instance-table", "selected_rows")],
    [Input("tur-opt-instance-table", "page_current"),
     Input("tur-opt-table-search-input", "value"),
     Input("tur-opt-table-filters", "data"),
     Input('tur-opt-scatter-plot', 'selectedData'),
     Input('tur-opt-selected-instances-store', 'data'),
     Input('tur-opt-instance-table', 'sort_by')],
    prevent_initial_call=False
)
def update_table_data_turkiye_opt(page_current, search_value, filters, selectedData, selected_instances, sort_by):
    """Table data update"""
    ...
    try:
        ...
        print(f"TUR OPT Error in table: {e}")
        import traceback
        traceback.print_exc()
...
@callback(
    [Output("tur-opt-instance-table", "page_current"),
     Output("tur-opt-table-current-page", "children"),
     Output("tur-opt-table-total-pages", "children"),
     Output("tur-opt-table-first-page-btn", "disabled"),
     Output("tur-opt-table-prev-page-btn", "disabled"),
     Output("tur-opt-table-next-page-btn", "disabled"),
     Output("tur-opt-table-last-page-btn", "disabled")],
    [Input("tur-opt-table-first-page-btn", "n_clicks"),
     Input("tur-opt-table-prev-page-btn", "n_clicks"),
     Input("tur-opt-table-next-page-btn", "n_clicks"),
     Input("tur-opt-table-last-page-btn", "n_clicks"),
     Input("tur-opt-table-search-input", "value"),
     Input("tur-opt-table-filters", "data"),
     Input('tur-opt-scatter-plot', 'selectedData'),
     Input('tur-opt-selected-instances-store', 'data')],
    [State("tur-opt-instance-table", "page_current")],
    prevent_initial_call=False
)
def handle_manual_pagination_turkiye_opt(first_clicks, prev_clicks, next_clicks, last_clicks,
    ...
    try:
        ...
        if ctx.triggered:
            ...
            if ("tur-opt-table-search-input" in triggered_prop or
                "tur-opt-table-filters" in triggered_prop or
                "tur-opt-scatter-plot.selectedData" in triggered_prop or
                "tur-opt-selected-instances-store" in triggered_prop):
                new_page = 0
            elif "tur-opt-table-first-page-btn" in triggered_prop and first_clicks:
                new_page = 0
            elif "tur-opt-table-prev-page-btn" in triggered_prop and prev_clicks:
                new_page = max(0, current_page - 1)
            elif "tur-opt-table-next-page-btn" in triggered_prop and next_clicks:
                new_page = min(total_pages - 1, current_page + 1)
            elif "tur-opt-table-last-page-btn" in triggered_prop and last_clicks:
        ...
        print(f"TUR OPT pagination error: {e}")
        return 0, "1", "1", True, True, True, True


@callback(
    Output('tur-opt-selected-instances-store', 'data', allow_duplicate=True),
    Input('tur-opt-instance-table', 'selected_rows'),
    [State('tur-opt-instance-table', 'data'),
     State('tur-opt-selected-instances-store', 'data')],
    prevent_initial_call=True
)
def update_selection_from_table_turkiye_opt(selected_rows, table_data, current_selected):
...
# ==========================================
# FILTER CHIPS - TURKIYE OPT
# ==========================================

@callback(
    Output('tur-opt-filter-chips-container', 'children'),
    Input('tur-opt-table-filters', 'data'),
    prevent_initial_call=False
)
def update_filter_chips_turkiye_opt(filters):
    ...
    for column, filter_value in filters.items():
        ...
        chip = dbc.Badge(
            [
                chip_text,
                html.Span(" ×",
                    id={"type": "tur-opt-remove-filter", "index": column},
                    style={
                        "marginLeft": "8px",
                        "cursor": "pointer",
                        "fontWeight": "bold",
...
@callback(
    Output('tur-opt-table-filters', 'data'),
    Input({"type": "tur-opt-remove-filter", "index": dash.ALL}, 'n_clicks'),
    State('tur-opt-table-filters', 'data'),
    prevent_initial_call=True
)
def remove_filter_turkiye_opt(n_clicks_list, current_filters):
    """Remove filter"""
...
@callback(
    Output('tur-opt-download-instances', 'data'),
    Input('tur-opt-export-selected-btn', 'n_clicks'),
    [State('tur-opt-scatter-plot', 'selectedData'),
     State('tur-opt-selected-instances-store', 'data'),
     State('tur-opt-table-search-input', 'value'),
     State('tur-opt-table-filters', 'data')],
    prevent_initial_call=True
)
def export_selected_instances_turkiye_opt(n_clicks, selectedData, selected_instances, search_value, filters):
    ...
    try:
        ...
        filename = f"TUR_OptimizedSolutions_{timestamp}.csv"
        
        return dcc.send_data_frame(export_df.to_csv, filename, index=False)
        
    except Exception as e:
        print(f"TUR OPT export error: {e}")
        return dash.no_update


# ==========================================
# FILTER DROPDOWN - TURKIYE OPT
# ==========================================

@callback(
    [Output('tur-opt-filter-dropdown-menu', 'style'),
     Output('tur-opt-filter-dropdown-open', 'data'),
     Output('tur-opt-add-filter-trigger-btn', 'className')],
    [Input('tur-opt-add-filter-trigger-btn', 'n_clicks')],
    [State('tur-opt-filter-dropdown-open', 'data')],
    prevent_initial_call=True
)
def toggle_filter_dropdown_turkiye_opt(n_clicks, is_open):
...
@callback(
    [Output('tur-opt-filter-step-1', 'style'),
     Output('tur-opt-filter-step-2', 'style'),
     Output('tur-opt-filter-current-step', 'data')],
    [Input({"type": "tur-opt-param-selector", "param": dash.ALL}, 'n_clicks'),
     Input('tur-opt-filter-back-btn', 'n_clicks')],
    [State('tur-opt-filter-current-step', 'data')],
    prevent_initial_call=True
)
def handle_filter_step_navigation_turkiye_opt(param_clicks, back_clicks, current_step):
    ...
    if 'tur-opt-filter-back-btn' in triggered_id:
        return (
            {"display": "block"},
            {"display": "none"},
            1
    ...
    elif 'tur-opt-param-selector' in triggered_id:
        return (
            {"display": "none"},
            {"display": "block"},
            2
...
@callback(
    [Output('tur-opt-filter-selected-param', 'data'),
     Output('tur-opt-filter-min-input', 'placeholder'),
     Output('tur-opt-filter-max-input', 'placeholder')],
    [Input({"type": "tur-opt-param-selector", "param": dash.ALL}, 'n_clicks')],
    [State({"type": "tur-opt-param-selector", "param": dash.ALL}, 'id')],
    prevent_initial_call=True
)
def handle_parameter_selection_turkiye_opt(n_clicks_list, param_ids):
...
@callback(
    [Output('tur-opt-table-filters', 'data', allow_duplicate=True),
     Output('tur-opt-filter-dropdown-menu', 'style', allow_duplicate=True),
     Output('tur-opt-filter-dropdown-open', 'data', allow_duplicate=True),
     Output('tur-opt-add-filter-trigger-btn', 'className', allow_duplicate=True)],
    [Input('tur-opt-filter-apply-btn', 'n_clicks')],
    [State('tur-opt-filter-selected-param', 'data'),
     State('tur-opt-filter-min-input', 'value'),
     State('tur-opt-filter-max-input', 'value'),
     State('tur-opt-table-filters', 'data')],
    prevent_initial_call=True
)
def apply_filter_from_dropdown_turkiye_opt(n_clicks, selected_param, min_val, max_val, current_filters):
    """Apply filter"""
...
# ==========================================
# CLIENTSIDE - WARNING - TURKIYE OPT
# ==========================================
...
clientside_callback(
    ...
    function(warning_state) {
        if (warning_state && warning_state.show) {
            ...
            window.turOptWarningTimeout = setTimeout(function() {
                if (window.dash_clientside && window.dash_clientside.set_props) {
                    try {
                        window.dash_clientside.set_props('tur-opt-selection-warning-state', {
                            data: {show: false, message: ''}
                        });
                    } catch (error) {
                        console.error('Error auto-clearing TUR OPT warning:', error);
                    }
                }
            }, 3000);
        }
    ...
    """,
    Output('tur-opt-selection-warning-state', 'data', allow_duplicate=True),
    Input('tur-opt-selection-warning-state', 'data'),
    prevent_initial_call=True
)
...
clientside_callback(
    ...
    function(warning_state, dropdown_state) {
        const overlay = document.getElementById('tur-opt-warning-overlay');
    ...
    """,
    Output('tur-opt-warning-overlay', 'data-status', allow_duplicate=True),
    [Input('tur-opt-selection-warning-state', 'data'),
     Input('tur-opt-dropdown-open-state', 'data')],
     prevent_initial_call=True
)
...
clientside_callback(
    ...
    """,
    Output('tur-opt-virtual-dropdown-trigger', 'data-keyboard-handler'),
    Input('tur-opt-virtual-dropdown-trigger', 'n_clicks'),
    prevent_initial_call=True
)
...
clientside_callback(
    ...
    function(n_clicks) {
        if (n_clicks > 0 && !window.turOptFilterDropdownClickListener) {
            setTimeout(function() {
                document.addEventListener('click', function(e) {
                    const trigger = document.getElementById('tur-opt-add-filter-trigger-btn');
                    const menu = document.getElementById('tur-opt-filter-dropdown-menu');
                    ...
                    if (trigger && menu && 
                        ...
                        if (menu.style.display === 'block') {
                            ...
                            if (window.dash_clientside && window.dash_clientside.set_props) {
                                try {
                                    window.dash_clientside.set_props('tur-opt-filter-dropdown-open', {data: false});
                                } catch (error) {
                                    console.error('Error closing TUR OPT filter dropdown:', error);
                                }
                            }
                        }
                    }
    ...
    """,
    Output('tur-opt-add-filter-trigger-btn', 'data-click-listener'),
    Input('tur-opt-add-filter-trigger-btn', 'n_clicks'),
    prevent_initial_call=True
)
...
# ==========================================
# CACHE MANAGEMENT - TURKIYE OPT
# ==========================================

def clear_caches_turkiye_opt():
    """Clear all caches"""
    global _dropdown_cache_turkiye_opt, _param_cache_turkiye_opt, _comparison_cache_turkiye_opt, _graph_cache_turkiye_opt
    _dropdown_cache_turkiye_opt.clear()
    _param_cache_turkiye_opt.clear()
    _comparison_cache_turkiye_opt.clear()
    _graph_cache_turkiye_opt.clear()
    print("TUR OPT: All caches cleared")

...
import threading
import re
from pages.spain_optimized import df
from dash import callback, Output, Input, ctx, html, dcc, State, clientside_callback, callback_context

# ==========================================
# PERFORMANCE OPTIMIZATIONS - SPAGARY OPT
# ==========================================

_cached_data_spain_opt = None
_cached_df_with_id_spain_opt = None

def get_cached_data_spain_opt():
    """Load Spain optimized data once and cache it"""
    global _cached_data_spain_opt, _cached_df_with_id_spain_opt
    if _cached_data_spain_opt is None:
        _cached_data_spain_opt = pd.read_csv("data/spain/pareto_front_predicted_100_spa.csv")
        ...
        for col in _cached_data_spain_opt.columns:
            if col not in baseline_spain:
                baseline_spain[col] = None
        
        baseline_df = pd.DataFrame([baseline_spain])
        _cached_data_spain_opt = pd.concat([baseline_df, _cached_data_spain_opt], ignore_index=True)
        ...
        _cached_data_spain_opt = _cached_data_spain_opt.rename(columns=column_mapping)
        
        # Add co2 column if missing
        if 'co2' not in _cached_data_spain_opt.columns:
            _cached_data_spain_opt['co2'] = 0
        
        _cached_df_with_id_spain_opt = _cached_data_spain_opt.copy()
        _cached_df_with_id_spain_opt['ID'] = _cached_df_with_id_spain_opt.index + 1
    return _cached_data_spain_opt, _cached_df_with_id_spain_opt

def get_selectable_data_spain_opt():
    """Get selectable data for optimized page"""
    df, df_with_id = get_cached_data_spain_opt()
...
# ==========================================
# CACHE DICTIONARIES - SPAGARY OPT
# ==========================================

_dropdown_cache_spain_opt = {}
_param_cache_spain_opt = {}
_comparison_cache_spain_opt = {}
_graph_cache_spain_opt = {}

def format_co2_value_spain_opt(co2_value):
    """Format CO2 value"""
    try:
...
# ==========================================
# VIRTUAL CONTENT UPDATE - SPAGARY OPT
# ==========================================

@callback(
    [Output('spa-opt-virtual-scroll-container', 'children'),
     Output('spa-opt-virtual-scroll-position', 'data')],
    [Input('spa-opt-virtual-scroll-container', 'scrollTop'),
     Input('spa-opt-comparison-selected-instances-store', 'data'),
     Input('spa-opt-sort-by-dropdown', 'value'),
     Input('spa-opt-sort-order-radio', 'value'),
     Input('spa-opt-virtual-scroll-position', 'data')],
    [State('spa-opt-virtual-item-height', 'data'),
     State('spa-opt-virtual-container-height', 'data')],
    prevent_initial_call=False
)
def update_virtual_content_combined_spain_opt(scroll_top_prop, selected_instances, sort_by, sort_order, scroll_position_data, item_height, container_height):
    """Virtual scrolling for Spain optimized"""
    ...
    try:
        ...
        df, df_with_id = get_cached_data_spain_opt()
        selectable_df = get_selectable_data_spain_opt()
        ...
        cache_key = f"spa_opt_{scroll_group}_{selected_key}_{sort_by}_{sort_order}"
        
        triggered_id = ctx.triggered_id
        
        if cache_key in _dropdown_cache_spain_opt and triggered_id != 'spa-opt-selected-instances-store':
            return _dropdown_cache_spain_opt[cache_key], scroll_top
        ...
        for i in range(visible_start, visible_end):
            ...
            unique_checkbox_id = f"spa-opt-virtual-checkbox-{instance_id}-{scroll_group}"
            ...
            visible_items.append(
                html.Div([
                    ...
                    html.Div([
                        dcc.Checklist(
                            id={"type": "spa-opt-virtual-checkbox", "index": instance_id, "unique": unique_checkbox_id},
                ...
                id=f"spa-opt-virtual-item-{instance_id}",
        ...
        if len(_dropdown_cache_spain_opt) > 50:
            old_keys = list(_dropdown_cache_spain_opt.keys())[:25]
            for key in old_keys:
                del _dropdown_cache_spain_opt[key]
        
        _dropdown_cache_spain_opt[cache_key] = visible_items
        
        return visible_items, scroll_top
        
    except Exception as e:
        print(f"SPA OPT Virtual content error: {e}")
        import traceback
        traceback.print_exc()
...
# ==========================================
# WARNING OVERLAY - SPAKIYE OPT
# ==========================================

@callback(
    [Output('spa-opt-warning-overlay', 'children'),
     Output('spa-opt-warning-overlay', 'style')],
    Input('spa-opt-selection-warning-state', 'data'),
    [State('spa-opt-dropdown-open-state', 'data')],
    prevent_initial_call=False
)
def update_warning_overlay_spain_opt(warning_state, dropdown_is_open):
...
@callback(
    Output('spa-opt-warning-overlay', 'style', allow_duplicate=True),
    Input('spa-opt-dropdown-open-state', 'data'),
    [State('spa-opt-selection-warning-state', 'data'),
     State('spa-opt-warning-overlay', 'style')],
    prevent_initial_call=True
)
def update_warning_position_on_dropdown_toggle_spain_opt(dropdown_is_open, warning_state, current_style):
...
# ==========================================
# SELECTION HANDLING - SPAKIYE OPT
# ==========================================

@callback(
    [Output('spa-opt-comparison-selected-instances-store', 'data'),
     Output('spa-opt-selection-warning-state', 'data')],
    [Input({"type": "spa-opt-virtual-checkbox", "index": dash.ALL, "unique": dash.ALL}, 'value'),
     Input('spa-opt-search-input', 'n_submit')],
    [State({"type": "spa-opt-virtual-checkbox", "index": dash.ALL, "unique": dash.ALL}, 'id'),
     State('spa-opt-comparison-selected-instances-store', 'data'),
     State('spa-opt-search-input', 'value')],
    prevent_initial_call=True
)
def handle_selection_and_search_with_warning_spain_opt(all_values, n_submit, all_ids, current_selected, search_value):
    """Selection handling for Spain optimized"""
    ...
    # Handle search input
    if triggered_id == 'spa-opt-search-input' and n_submit and search_value is not None:
        try:
            ...
            if 1 <= instance_id <= 100:
                ...
                if instance_id not in new_selected:
                    ...
                    print(f"SPA OPT SEARCH: Added S{instance_id}")
                
                def delayed_cache_clear():
                    time.sleep(0.1)
                    global _dropdown_cache_spain_opt
                    _dropdown_cache_spain_opt.clear()
    ...
    if all_values and all_ids:
        try:
            ...
            if 'spa-opt-virtual-checkbox' not in prop_id:
                raise PreventUpdate
            ...
            print(f"SPA OPT Selection error: {e}")
    
    return current_selected, warning_state


# ==========================================
# DROPDOWN STATE - SPAKIYE OPT
# ==========================================

@callback(
    [Output('spa-opt-virtual-scroll-container', 'style'),
     Output('spa-opt-dropdown-open-state', 'data'),
     Output('spa-opt-virtual-dropdown-trigger', 'children')],
    [Input('spa-opt-virtual-dropdown-trigger', 'n_clicks'),
     Input('spa-opt-comparison-selected-instances-store', 'data')],
    [State('spa-opt-dropdown-open-state', 'data')],
    prevent_initial_call=True
)
def handle_dropdown_state_combined_spain_opt(n_clicks, selected_instances, is_open):
    """Dropdown toggle"""
    
    triggered_id = ctx.triggered_id
    
    if triggered_id == 'spa-opt-virtual-dropdown-trigger' and n_clicks:
        ...
        trigger_text = get_trigger_text_spain_opt(selected_instances)
        return style, new_state, trigger_text
    
    if triggered_id == 'spa-opt-selected-instances-store':
        current_style = {
...
def get_trigger_text_spain_opt(selected_instances):
...
# ==========================================
# SEARCH TOGGLE - SPAKIYE OPT
# ==========================================

@callback(
    [Output('spa-opt-search-input-container', 'className'),
     Output('spa-opt-search-toggle-state', 'data'),
     Output('spa-opt-search-input', 'value'),
     Output('spa-opt-search-toggle-button', 'className')],
    [Input('spa-opt-search-toggle-button', 'n_clicks'),
     Input('spa-opt-search-input', 'n_submit')],
    [State('spa-opt-search-toggle-state', 'data'),
     State('spa-opt-search-input', 'value')],
    prevent_initial_call=True
)
def handle_search_toggle_combined_spain_opt(toggle_clicks, submit_count, is_open, search_value):
    """Handle search visibility"""
    
    triggered_id = ctx.triggered_id
    
    if triggered_id == 'spa-opt-search-toggle-button':
    ...
    if triggered_id == 'spa-opt-search-input' and search_value:
...
# ==========================================
# PARAMETER UPDATE - SPAKIYE OPT
# ==========================================

@callback(
    Output('spa-opt-instance-parameters', 'children'),
    Input('spa-opt-comparison-selected-instances-store', 'data'),
    prevent_initial_call=False
)
def update_instance_parameters_optimized_spain_opt(selected_instances):
    """Parameter updates with caching"""
    
    df, _ = get_cached_data_spain_opt()
    ...
    cache_key = f"spa_opt_{selected_id}_baseline_{baseline_mode}"
    
    if cache_key in _param_cache_spain_opt:
        return _param_cache_spain_opt[cache_key]
    ...
    _param_cache_spain_opt[cache_key] = result
    return result


# ==========================================
# COMPARISON UPDATE - SPAKIYE OPT
# ==========================================

@callback(
    Output('spa-opt-instance-comparison-content', 'children'),
    Input('spa-opt-comparison-selected-instances-store', 'data'),
    prevent_initial_call=False
)
def update_instance_comparison_optimized_spain_opt(selected_instances):
    ...
    def format_value(raw_val, metric_key=None):
        try:
            if metric_key == "co2":
                return format_co2_value_spain_opt(raw_val)
            else:
                if pd.isna(raw_val):
    ...
    def create_instance_content_with_remove_btn(instance_label, instance_id):
        ...
        return html.Div([
            html.Div([
                ...
                html.Button(
                    "×", 
                    id={"type": "spa-opt-remove-instance-btn", "index": instance_id},
...
# ==========================================
# REMOVE BUTTON - SPAKIYE OPT
# ==========================================

@callback(
    Output('spa-opt-comparison-selected-instances-store', 'data', allow_duplicate=True),
    Input({"type": "spa-opt-remove-instance-btn", "index": dash.ALL}, 'n_clicks'),
    State('spa-opt-comparison-selected-instances-store', 'data'),
    prevent_initial_call=True
)
def handle_remove_instance_button_spain_opt(n_clicks_list, current_selected):
    ...
    global _dropdown_cache_spain_opt, _comparison_cache_spain_opt, _graph_cache_spain_opt
    _dropdown_cache_spain_opt.clear()
    _comparison_cache_spain_opt.clear()
    _graph_cache_spain_opt.clear()
    
    return new_selected


# ==========================================
# GRAPH UPDATE - SPAKIYE OPT
# ==========================================

@callback(
    Output('spa-opt-comparison-graph', 'figure'),
    Input('spa-opt-comparison-selected-instances-store', 'data'),
    prevent_initial_call=False
)
def update_comparison_graph_optimized_spain_opt(selected_instances):
    """Graph updates"""
    ...
    cache_key = "spa_opt_" + "_".join(map(str, sorted(selected_instances)))
    
    if cache_key in _graph_cache_spain_opt:
        return _graph_cache_spain_opt[cache_key]
    ...
    _graph_cache_spain_opt[cache_key] = fig
    return fig


# ==========================================
# CLIENTSIDE CALLBACKS - SPAKIYE OPT
# ==========================================
...
clientside_callback(
    ...
    function(n_clicks) {
        if (n_clicks > 0) {
            setTimeout(function() {
                const container = document.getElementById('spa-opt-virtual-scroll-container');
                        ...
                        scrollTimeout = setTimeout(function() {
                            if (Math.abs(scrollTop - lastScrollTop) > 3) {
                                ...
                                if (window.dash_clientside && window.dash_clientside.set_props) {
                                    try {
                                        window.dash_clientside.set_props('spa-opt-virtual-scroll-position', {
                                            data: scrollTop
                                        });
                                    } catch (error) {
                                        console.error('SPA OPT scroll error:', error);
                                    }
                                }
                            }
                        }, 30);
    ...
    """,
    Output('spa-opt-virtual-scroll-container', 'data-scroll-initialized'),
    Input('spa-opt-virtual-dropdown-trigger', 'n_clicks'),
    prevent_initial_call=True
)
...
clientside_callback(
        ...
        if (n_clicks > 0 && !window.turOptOutsideClickAttached) {
            setTimeout(function() {
                document.addEventListener('click', function(e) {
                    const dropdown = document.getElementById('spa-opt-virtual-dropdown-trigger');
                    const menu = document.getElementById('spa-opt-virtual-scroll-container');
                    const warning = document.getElementById('spa-opt-warning-overlay');
                    ...
                    if (dropdown && menu && 
                        ...
                        if (menu.style.display === 'block') {
                            ...
                            if (window.dash_clientside && window.dash_clientside.set_props) {
                                window.dash_clientside.set_props('spa-opt-dropdown-open-state', {data: false});
                            }
                        }
                    }
                }, { passive: true });
    ...
    """,
    Output('spa-opt-dummy-outside-click', 'children'),
    Input('spa-opt-virtual-dropdown-trigger', 'n_clicks'),
    prevent_initial_call=True
)

@callback(
    [Output('spa-opt-virtual-item-height', 'data'),
     Output('spa-opt-virtual-container-height', 'data')],
    Input('spa-opt-virtual-dropdown-trigger', 'n_clicks'),
    prevent_initial_call=False
)
def validate_virtual_dimensions_spain_opt(n_clicks):
    """Validate dimensions"""
    return 40, 320


@callback(
    Output('spa-opt-virtual-scroll-container', 'data-total-items'),
    Input('spa-opt-sort-by-dropdown', 'value'),
    prevent_initial_call=False
)
def validate_total_items_spain_opt(sort_by):
...
# ==========================================
# TABLE CALLBACKS - SPAKIYE OPT
# ==========================================

@callback(
    [Output("spa-opt-instance-table", "data"),
     Output("spa-opt-table-info", "children"),
     Output("spa-opt-instance-table", "selected_rows")],
    [Input("spa-opt-instance-table", "page_current"),
     Input("spa-opt-table-search-input", "value"),
     Input("spa-opt-table-filters", "data"),
     Input('spa-opt-scatter-plot', 'selectedData'),
     Input('spa-opt-selected-instances-store', 'data'),
     Input('spa-opt-instance-table', 'sort_by')],
    prevent_initial_call=False
)
def update_table_data_spain_opt(page_current, search_value, filters, selectedData, selected_instances, sort_by):
    """Table data update"""
    ...
    try:
        ...
        print(f"SPA OPT Error in table: {e}")
        import traceback
        traceback.print_exc()
...
@callback(
    [Output("spa-opt-instance-table", "page_current"),
     Output("spa-opt-table-current-page", "children"),
     Output("spa-opt-table-total-pages", "children"),
     Output("spa-opt-table-first-page-btn", "disabled"),
     Output("spa-opt-table-prev-page-btn", "disabled"),
     Output("spa-opt-table-next-page-btn", "disabled"),
     Output("spa-opt-table-last-page-btn", "disabled")],
    [Input("spa-opt-table-first-page-btn", "n_clicks"),
     Input("spa-opt-table-prev-page-btn", "n_clicks"),
     Input("spa-opt-table-next-page-btn", "n_clicks"),
     Input("spa-opt-table-last-page-btn", "n_clicks"),
     Input("spa-opt-table-search-input", "value"),
     Input("spa-opt-table-filters", "data"),
     Input('spa-opt-scatter-plot', 'selectedData'),
     Input('spa-opt-selected-instances-store', 'data')],
    [State("spa-opt-instance-table", "page_current")],
    prevent_initial_call=False
)
def handle_manual_pagination_spain_opt(first_clicks, prev_clicks, next_clicks, last_clicks,
    ...
    try:
        ...
        if ctx.triggered:
            ...
            if ("spa-opt-table-search-input" in triggered_prop or
                "spa-opt-table-filters" in triggered_prop or
                "spa-opt-scatter-plot.selectedData" in triggered_prop or
                "spa-opt-selected-instances-store" in triggered_prop):
                new_page = 0
            elif "spa-opt-table-first-page-btn" in triggered_prop and first_clicks:
                new_page = 0
            elif "spa-opt-table-prev-page-btn" in triggered_prop and prev_clicks:
                new_page = max(0, current_page - 1)
            elif "spa-opt-table-next-page-btn" in triggered_prop and next_clicks:
                new_page = min(total_pages - 1, current_page + 1)
            elif "spa-opt-table-last-page-btn" in triggered_prop and last_clicks:
        ...
        print(f"SPA OPT pagination error: {e}")
        return 0, "1", "1", True, True, True, True


@callback(
    Output('spa-opt-selected-instances-store', 'data', allow_duplicate=True),
    Input('spa-opt-instance-table', 'selected_rows'),
    [State('spa-opt-instance-table', 'data'),
     State('spa-opt-selected-instances-store', 'data')],
    prevent_initial_call=True
)
def update_selection_from_table_spain_opt(selected_rows, table_data, current_selected):
...
# ==========================================
# FILTER CHIPS - SPAKIYE OPT
# ==========================================

@callback(
    Output('spa-opt-filter-chips-container', 'children'),
    Input('spa-opt-table-filters', 'data'),
    prevent_initial_call=False
)
def update_filter_chips_spain_opt(filters):
    ...
    for column, filter_value in filters.items():
        ...
        chip = dbc.Badge(
            [
                chip_text,
                html.Span(" ×",
                    id={"type": "spa-opt-remove-filter", "index": column},
                    style={
                        "marginLeft": "8px",
                        "cursor": "pointer",
                        "fontWeight": "bold",
...
@callback(
    Output('spa-opt-table-filters', 'data'),
    Input({"type": "spa-opt-remove-filter", "index": dash.ALL}, 'n_clicks'),
    State('spa-opt-table-filters', 'data'),
    prevent_initial_call=True
)
def remove_filter_spain_opt(n_clicks_list, current_filters):
    """Remove filter"""
...
@callback(
    Output('spa-opt-download-instances', 'data'),
    Input('spa-opt-export-selected-btn', 'n_clicks'),
    [State('spa-opt-scatter-plot', 'selectedData'),
     State('spa-opt-selected-instances-store', 'data'),
     State('spa-opt-table-search-input', 'value'),
     State('spa-opt-table-filters', 'data')],
    prevent_initial_call=True
)
def export_selected_instances_spain_opt(n_clicks, selectedData, selected_instances, search_value, filters):
    ...
    try:
        ...
        filename = f"SPA_OptimizedSolutions_{timestamp}.csv"
        
        return dcc.send_data_frame(export_df.to_csv, filename, index=False)
        
    except Exception as e:
        print(f"SPA OPT export error: {e}")
        return dash.no_update


# ==========================================
# FILTER DROPDOWN - SPAKIYE OPT
# ==========================================

@callback(
    [Output('spa-opt-filter-dropdown-menu', 'style'),
     Output('spa-opt-filter-dropdown-open', 'data'),
     Output('spa-opt-add-filter-trigger-btn', 'className')],
    [Input('spa-opt-add-filter-trigger-btn', 'n_clicks')],
    [State('spa-opt-filter-dropdown-open', 'data')],
    prevent_initial_call=True
)
def toggle_filter_dropdown_spain_opt(n_clicks, is_open):
...
@callback(
    [Output('spa-opt-filter-step-1', 'style'),
     Output('spa-opt-filter-step-2', 'style'),
     Output('spa-opt-filter-current-step', 'data')],
    [Input({"type": "spa-opt-param-selector", "param": dash.ALL}, 'n_clicks'),
     Input('spa-opt-filter-back-btn', 'n_clicks')],
    [State('spa-opt-filter-current-step', 'data')],
    prevent_initial_call=True
)
def handle_filter_step_navigation_spain_opt(param_clicks, back_clicks, current_step):
    ...
    if 'spa-opt-filter-back-btn' in triggered_id:
        return (
            {"display": "block"},
            {"display": "none"},
            1
    ...
    elif 'spa-opt-param-selector' in triggered_id:
        return (
            {"display": "none"},
            {"display": "block"},
            2
...
@callback(
    [Output('spa-opt-filter-selected-param', 'data'),
     Output('spa-opt-filter-min-input', 'placeholder'),
     Output('spa-opt-filter-max-input', 'placeholder')],
    [Input({"type": "spa-opt-param-selector", "param": dash.ALL}, 'n_clicks')],
    [State({"type": "spa-opt-param-selector", "param": dash.ALL}, 'id')],
    prevent_initial_call=True
)
def handle_parameter_selection_spain_opt(n_clicks_list, param_ids):
...
@callback(
    [Output('spa-opt-table-filters', 'data', allow_duplicate=True),
     Output('spa-opt-filter-dropdown-menu', 'style', allow_duplicate=True),
     Output('spa-opt-filter-dropdown-open', 'data', allow_duplicate=True),
     Output('spa-opt-add-filter-trigger-btn', 'className', allow_duplicate=True)],
    [Input('spa-opt-filter-apply-btn', 'n_clicks')],
    [State('spa-opt-filter-selected-param', 'data'),
     State('spa-opt-filter-min-input', 'value'),
     State('spa-opt-filter-max-input', 'value'),
     State('spa-opt-table-filters', 'data')],
    prevent_initial_call=True
)
def apply_filter_from_dropdown_spain_opt(n_clicks, selected_param, min_val, max_val, current_filters):
    """Apply filter"""
...
# ==========================================
# CLIENTSIDE - WARNING - SPAKIYE OPT
# ==========================================
...
clientside_callback(
    ...
    function(warning_state) {
        if (warning_state && warning_state.show) {
            ...
            window.turOptWarningTimeout = setTimeout(function() {
                if (window.dash_clientside && window.dash_clientside.set_props) {
                    try {
                        window.dash_clientside.set_props('spa-opt-selection-warning-state', {
                            data: {show: false, message: ''}
                        });
                    } catch (error) {
                        console.error('Error auto-clearing SPA OPT warning:', error);
                    }
                }
            }, 3000);
        }
    ...
    """,
    Output('spa-opt-selection-warning-state', 'data', allow_duplicate=True),
    Input('spa-opt-selection-warning-state', 'data'),
    prevent_initial_call=True
)
...
clientside_callback(
    ...
    function(warning_state, dropdown_state) {
        const overlay = document.getElementById('spa-opt-warning-overlay');
    ...
    """,
    Output('spa-opt-warning-overlay', 'data-status', allow_duplicate=True),
    [Input('spa-opt-selection-warning-state', 'data'),
     Input('spa-opt-dropdown-open-state', 'data')],
     prevent_initial_call=True
)
...
clientside_callback(
    ...
    """,
    Output('spa-opt-virtual-dropdown-trigger', 'data-keyboard-handler'),
    Input('spa-opt-virtual-dropdown-trigger', 'n_clicks'),
    prevent_initial_call=True
)
...
clientside_callback(
    ...
    function(n_clicks) {
        if (n_clicks > 0 && !window.turOptFilterDropdownClickListener) {
            setTimeout(function() {
                document.addEventListener('click', function(e) {
                    const trigger = document.getElementById('spa-opt-add-filter-trigger-btn');
                    const menu = document.getElementById('spa-opt-filter-dropdown-menu');
                    ...
                    if (trigger && menu && 
                        ...
                        if (menu.style.display === 'block') {
                            ...
                            if (window.dash_clientside && window.dash_clientside.set_props) {
                                try {
                                    window.dash_clientside.set_props('spa-opt-filter-dropdown-open', {data: false});
                                } catch (error) {
                                    console.error('Error closing SPA OPT filter dropdown:', error);
                                }
                            }
                        }
                    }
    ...
    """,
    Output('spa-opt-add-filter-trigger-btn', 'data-click-listener'),
    Input('spa-opt-add-filter-trigger-btn', 'n_clicks'),
    prevent_initial_call=True
)
...
# ==========================================
# CACHE MANAGEMENT - SPAKIYE OPT
# ==========================================

def clear_caches_spain_opt():
    """Clear all caches"""
    global _dropdown_cache_spain_opt, _param_cache_spain_opt, _comparison_cache_spain_opt, _graph_cache_spain_opt
    _dropdown_cache_spain_opt.clear()
    _param_cache_spain_opt.clear()
    _comparison_cache_spain_opt.clear()
    _graph_cache_spain_opt.clear()
    print("SPA OPT: All caches cleared")

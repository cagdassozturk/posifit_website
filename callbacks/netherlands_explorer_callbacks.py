...
# Import from netherlands_explorer page
from pages.netherlands_explorer import df, labels

# Modal data - Netherlands image paths
MODAL_IMAGES_NETKIYE = [
    {
        "id": "net-plot-wall-u",
...
@callback(
    [Output("net-parallel-plot", "figure"),
     Output("net-scatter-plot", "figure")],
    [Input("net-scatter-plot", "clickData"),
     Input("net-scatter-plot", "selectedData"),
     Input("net-parallel-plot", "restyleData")],
    [State("net-parallel-plot", "figure"),
     State("net-scatter-plot", "figure")],
    prevent_initial_call=True
)
def update_linked_graphs_netherlands(scatter_click, scatter_selected, parallel_restyle, parallel_fig, scatter_fig):
    """Link scatter and parallel coordinates plots for Netherlands explorer"""
    ...
    if "net-parallel-plot.restyleData" in triggered_prop:
    ...
    elif "net-scatter-plot.selectedData" in triggered_prop and scatter_selected and scatter_selected.get("points"):
        selected_indices = [pt["pointIndex"] for pt in scatter_selected.get("points")]
    
    elif "net-scatter-plot.clickData" in triggered_prop and scatter_click and scatter_click.get("points"):
...
@callback(
    [Output("net-image-modal", "style"),
     Output("net-modal-image", "src"),
     Output("net-modal-title", "children"),
     Output("net-current-image-index", "data"),
     Output("net-modal-prev", "disabled"),
     Output("net-modal-next", "disabled")],
    [Input("net-plot-wall-u", "n_clicks"),
     Input("net-plot-window-u", "n_clicks"),
     Input("net-plot-window-shgc", "n_clicks"),
     Input("net-plot-roof-u", "n_clicks"),
     Input("net-plot-transmittance", "n_clicks"),
     Input("net-modal-close", "n_clicks"),
     Input("net-modal-prev", "n_clicks"),
     Input("net-modal-next", "n_clicks")],
    [State("net-current-image-index", "data")],
    prevent_initial_call=True
)
def handle_modal_interactions_netherlands(wall_clicks, window_u_clicks, shgc_clicks, roof_clicks, 
                            trans_clicks, close_clicks, prev_clicks, next_clicks, 
                            current_index):
    """Handle all modal interactions for Netherlands explorer"""
    ...
    # Close modal
    if "net-modal-close" in triggered_prop:
        return {"display": "none"}, "", "", 0, False, False
    
    # Navigation - Previous
    if "net-modal-prev" in triggered_prop and prev_clicks:
        if current_index > 0:
            new_index = current_index - 1
            image_data = MODAL_IMAGES_NETKIYE[new_index]
    ...
    # Navigation - Next
    if "net-modal-next" in triggered_prop and next_clicks:
        if current_index < len(MODAL_IMAGES_NETKIYE) - 1:
            new_index = current_index + 1
            image_data = MODAL_IMAGES_NETKIYE[new_index]
            prev_disabled = False
            next_disabled = (new_index == len(MODAL_IMAGES_NETKIYE) - 1)
            return (
                {"display": "flex"}, 
                image_data["fullsize"], 
                image_data["title"],
    ...
    for i, image_data in enumerate(MODAL_IMAGES_NETKIYE):
        if image_data["id"] in triggered_prop:
            prev_disabled = (i == 0)
            next_disabled = (i == len(MODAL_IMAGES_NETKIYE) - 1)
...
clientside_callback(
    ...
    function(modal_style) {
        if (modal_style && modal_style.display === 'flex') {
            ...
            window.turModalKeyListener = function(e) {
                const modal = document.getElementById('net-image-modal');
                if (modal && modal.style.display === 'flex') {
                    if (e.key === 'Escape') {
                        document.getElementById('net-modal-close').click();
                    } else if (e.key === 'ArrowLeft') {
                        document.getElementById('net-modal-prev').click();
                        e.preventDefault();
                    } else if (e.key === 'ArrowRight') {
                        document.getElementById('net-modal-next').click();
                        e.preventDefault();
                    }
                }
            };
    ...
    """,
    Output('net-keyboard-listener', 'value'),
    Input('net-image-modal', 'style'),
    prevent_initial_call=True
)

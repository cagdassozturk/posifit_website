...
from pages.turkiye_market import (
    df_plot,
    df_search,
...
@callback(
    [Output('tur-market-selected-point-store', 'data'),
     Output('tur-market-scatter-plot', 'figure')],
    Input('tur-market-scatter-plot', 'clickData'),
    State('tur-market-scatter-plot', 'figure'),
    prevent_initial_call=True,
)
def tur_market_update_selected_point(clickData, current_fig):
    if not clickData:
        raise PreventUpdate
...
@callback(
    [Output('tur-market-spider-chart', 'figure'),
     Output('tur-market-instances-store', 'data')],
    [Input('tur-market-selected-point-store', 'data'),
     Input('tur-market-neighbors-slider', 'value')],
)
def tur_market_update_spider(selected_idx, n_neighbors):
...
@callback(
    [Output('tur-market-instance-table', 'data'),
     Output('tur-market-table-info', 'children'),
     Output('tur-market-layers-data', 'data')],
    [Input('tur-market-instances-store', 'data'),
     Input('tur-market-selected-point-store', 'data'),
     Input('tur-market-neighbors-slider', 'value')],
    prevent_initial_call=False,
)
def tur_market_update_table(instance_indices, selected_idx, n_neighbors):
...
@callback(
    Output('tur-market-download-instances', 'data'),
    Input('tur-export-market-button', 'n_clicks'),
    [State('tur-market-instance-table', 'selected_rows'),
     State('tur-market-instance-table', 'data')],
    prevent_initial_call=True,
)
def tur_market_export_selected(n_clicks, selected_rows, table_data):
    ...
    filename = f"TUR_MarketAvailableScenarios_All_{ts}.csv"
    return dcc.send_data_frame(export_df.to_csv, filename, index=False)


@callback(
    Output('tur-market-instance-table', 'selected_rows'),
    Input('tur-market-instance-table', 'data'),
    prevent_initial_call=False,
)
def tur_market_default_selection(table_data):
...
clientside_callback(
    """
    function(layersData) {
        // identical pattern to Luxembourg, but bound to Türkiye market table
        setTimeout(function() {
            document
    ...
    """,
    Output('tur-market-hover-state', 'data'),
    Input('tur-market-layers-data', 'data'),
)
...
clientside_callback(
    ...
    """,
    Output('tur-market-hover-state', 'data', allow_duplicate=True),
    Input('tur-market-instance-table', 'data'),
    prevent_initial_call=True,
)

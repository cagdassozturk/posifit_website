...
import threading
import re
from pages.hungary_optimized import df
from dash import callback, Output, Input, ctx, html, dcc, State, clientside_callback, callback_context

# ==========================================
# PERFORMANCE OPTIMIZATIONS - HUNGARY OPT
# ==========================================

_cached_data_hungary_opt = None
_cached_df_with_id_hungary_opt = None

def get_cached_data_hungary_opt():
    """Load Hungary optimized data once and cache it"""
    global _cached_data_hungary_opt, _cached_df_with_id_hungary_opt
    if _cached_data_hungary_opt is None:
        _cached_data_hungary_opt = pd.read_csv("data/hungary/pareto_front_predicted_100_hun.csv")
        ...
        for col in _cached_data_hungary_opt.columns:
            if col not in baseline_hungary:
                baseline_hungary[col] = None
        
        baseline_df = pd.DataFrame([baseline_hungary])
        _cached_data_hungary_opt = pd.concat([baseline_df, _cached_data_hungary_opt], ignore_index=True)
        ...
        _cached_data_hungary_opt = _cached_data_hungary_opt.rename(columns=column_mapping)
        
        # Add co2 column if missing
        if 'co2' not in _cached_data_hungary_opt.columns:
            _cached_data_hungary_opt['co2'] = 0
        
        _cached_df_with_id_hungary_opt = _cached_data_hungary_opt.copy()
        _cached_df_with_id_hungary_opt['ID'] = _cached_df_with_id_hungary_opt.index + 1
    return _cached_data_hungary_opt, _cached_df_with_id_hungary_opt

def get_selectable_data_hungary_opt():
    """Get selectable data for optimized page"""
    df, df_with_id = get_cached_data_hungary_opt()
...
# ==========================================
# CACHE DICTIONARIES - HUNGARY OPT
# ==========================================

_dropdown_cache_hungary_opt = {}
_param_cache_hungary_opt = {}
_comparison_cache_hungary_opt = {}
_graph_cache_hungary_opt = {}

def format_co2_value_hungary_opt(co2_value):
    """Format CO2 value"""
    try:
...
# ==========================================
# VIRTUAL CONTENT UPDATE - HUNGARY OPT
# ==========================================

@callback(
    [Output('hun-opt-virtual-scroll-container', 'children'),
     Output('hun-opt-virtual-scroll-position', 'data')],
    [Input('hun-opt-virtual-scroll-container', 'scrollTop'),
     Input('hun-opt-comparison-selected-instances-store', 'data'),
     Input('hun-opt-sort-by-dropdown', 'value'),
     Input('hun-opt-sort-order-radio', 'value'),
     Input('hun-opt-virtual-scroll-position', 'data')],
    [State('hun-opt-virtual-item-height', 'data'),
     State('hun-opt-virtual-container-height', 'data')],
    prevent_initial_call=False
)
def update_virtual_content_combined_hungary_opt(scroll_top_prop, selected_instances, sort_by, sort_order, scroll_position_data, item_height, container_height):
    """Virtual scrolling for Hungary optimized"""
    ...
    try:
        ...
        df, df_with_id = get_cached_data_hungary_opt()
        selectable_df = get_selectable_data_hungary_opt()
        ...
        cache_key = f"hun_opt_{scroll_group}_{selected_key}_{sort_by}_{sort_order}"
        
        triggered_id = ctx.triggered_id
        
        if cache_key in _dropdown_cache_hungary_opt and triggered_id != 'hun-opt-selected-instances-store':
            return _dropdown_cache_hungary_opt[cache_key], scroll_top
        ...
        for i in range(visible_start, visible_end):
            ...
            unique_checkbox_id = f"hun-opt-virtual-checkbox-{instance_id}-{scroll_group}"
            ...
            visible_items.append(
                html.Div([
                    ...
                    html.Div([
                        dcc.Checklist(
                            id={"type": "hun-opt-virtual-checkbox", "index": instance_id, "unique": unique_checkbox_id},
                ...
                id=f"hun-opt-virtual-item-{instance_id}",
        ...
        if len(_dropdown_cache_hungary_opt) > 50:
            old_keys = list(_dropdown_cache_hungary_opt.keys())[:25]
            for key in old_keys:
                del _dropdown_cache_hungary_opt[key]
        
        _dropdown_cache_hungary_opt[cache_key] = visible_items
        
        return visible_items, scroll_top
        
    except Exception as e:
        print(f"HUN OPT Virtual content error: {e}")
        import traceback
        traceback.print_exc()
...
# ==========================================
# WARNING OVERLAY - HUNKIYE OPT
# ==========================================

@callback(
    [Output('hun-opt-warning-overlay', 'children'),
     Output('hun-opt-warning-overlay', 'style')],
    Input('hun-opt-selection-warning-state', 'data'),
    [State('hun-opt-dropdown-open-state', 'data')],
    prevent_initial_call=False
)
def update_warning_overlay_hungary_opt(warning_state, dropdown_is_open):
...
@callback(
    Output('hun-opt-warning-overlay', 'style', allow_duplicate=True),
    Input('hun-opt-dropdown-open-state', 'data'),
    [State('hun-opt-selection-warning-state', 'data'),
     State('hun-opt-warning-overlay', 'style')],
    prevent_initial_call=True
)
def update_warning_position_on_dropdown_toggle_hungary_opt(dropdown_is_open, warning_state, current_style):
...
# ==========================================
# SELECTION HANDLING - HUNKIYE OPT
# ==========================================

@callback(
    [Output('hun-opt-comparison-selected-instances-store', 'data'),
     Output('hun-opt-selection-warning-state', 'data')],
    [Input({"type": "hun-opt-virtual-checkbox", "index": dash.ALL, "unique": dash.ALL}, 'value'),
     Input('hun-opt-search-input', 'n_submit')],
    [State({"type": "hun-opt-virtual-checkbox", "index": dash.ALL, "unique": dash.ALL}, 'id'),
     State('hun-opt-comparison-selected-instances-store', 'data'),
     State('hun-opt-search-input', 'value')],
    prevent_initial_call=True
)
def handle_selection_and_search_with_warning_hungary_opt(all_values, n_submit, all_ids, current_selected, search_value):
    """Selection handling for Hungary optimized"""
    ...
    # Handle search input
    if triggered_id == 'hun-opt-search-input' and n_submit and search_value is not None:
        try:
            instance_id = int(search_value)
            if 1 <= instance_id <= 101:
                ...
                if instance_id not in new_selected:
                    ...
                    print(f"HUN OPT SEARCH: Added S{instance_id}")
                
                def delayed_cache_clear():
                    time.sleep(0.1)
                    global _dropdown_cache_hungary_opt
                    _dropdown_cache_hungary_opt.clear()
    ...
    if all_values and all_ids:
        try:
            ...
            if 'hun-opt-virtual-checkbox' not in prop_id:
                raise PreventUpdate
            ...
            print(f"HUN OPT Selection error: {e}")
    
    return current_selected, warning_state


# ==========================================
# DROPDOWN STATE - HUNKIYE OPT
# ==========================================

@callback(
    [Output('hun-opt-virtual-scroll-container', 'style'),
     Output('hun-opt-dropdown-open-state', 'data'),
     Output('hun-opt-virtual-dropdown-trigger', 'children')],
    [Input('hun-opt-virtual-dropdown-trigger', 'n_clicks'),
     Input('hun-opt-comparison-selected-instances-store', 'data')],
    [State('hun-opt-dropdown-open-state', 'data')],
    prevent_initial_call=True
)
def handle_dropdown_state_combined_hungary_opt(n_clicks, selected_instances, is_open):
    """Dropdown toggle"""
    
    triggered_id = ctx.triggered_id
    
    if triggered_id == 'hun-opt-virtual-dropdown-trigger' and n_clicks:
        ...
        trigger_text = get_trigger_text_hungary_opt(selected_instances)
        return style, new_state, trigger_text
    
    if triggered_id == 'hun-opt-selected-instances-store':
        current_style = {
...
def get_trigger_text_hungary_opt(selected_instances):
...
# ==========================================
# SEARCH TOGGLE - HUNKIYE OPT
# ==========================================

@callback(
    [Output('hun-opt-search-input-container', 'className'),
     Output('hun-opt-search-toggle-state', 'data'),
     Output('hun-opt-search-input', 'value'),
     Output('hun-opt-search-toggle-button', 'className')],
    [Input('hun-opt-search-toggle-button', 'n_clicks'),
     Input('hun-opt-search-input', 'n_submit')],
    [State('hun-opt-search-toggle-state', 'data'),
     State('hun-opt-search-input', 'value')],
    prevent_initial_call=True
)
def handle_search_toggle_combined_hungary_opt(toggle_clicks, submit_count, is_open, search_value):
    """Handle search visibility"""
    
    triggered_id = ctx.triggered_id
    
    if triggered_id == 'hun-opt-search-toggle-button':
    ...
    if triggered_id == 'hun-opt-search-input' and search_value:
...
# ==========================================
# PARAMETER UPDATE - HUNKIYE OPT
# ==========================================

@callback(
    Output('hun-opt-instance-parameters', 'children'),
    Input('hun-opt-comparison-selected-instances-store', 'data'),
    prevent_initial_call=False
)
def update_instance_parameters_optimized_hungary_opt(selected_instances):
    """Parameter updates with caching"""
    
    df, _ = get_cached_data_hungary_opt()
    ...
    cache_key = f"hun_opt_{selected_id}_baseline_{baseline_mode}"
    
    if cache_key in _param_cache_hungary_opt:
        return _param_cache_hungary_opt[cache_key]
    ...
    _param_cache_hungary_opt[cache_key] = result
    return result


# ==========================================
# COMPARISON UPDATE - HUNKIYE OPT
# ==========================================

@callback(
    Output('hun-opt-instance-comparison-content', 'children'),
    Input('hun-opt-comparison-selected-instances-store', 'data'),
    prevent_initial_call=False
)
def update_instance_comparison_optimized_hungary_opt(selected_instances):
    ...
    def format_value(raw_val, metric_key=None):
        try:
            if metric_key == "co2":
                return format_co2_value_hungary_opt(raw_val)
            else:
                if pd.isna(raw_val):
    ...
    def create_instance_content_with_remove_btn(instance_label, instance_id):
        ...
        return html.Div([
            html.Div([
                ...
                html.Button(
                    "×", 
                    id={"type": "hun-opt-remove-instance-btn", "index": instance_id},
...
# ==========================================
# REMOVE BUTTON - HUNKIYE OPT
# ==========================================

@callback(
    Output('hun-opt-comparison-selected-instances-store', 'data', allow_duplicate=True),
    Input({"type": "hun-opt-remove-instance-btn", "index": dash.ALL}, 'n_clicks'),
    State('hun-opt-comparison-selected-instances-store', 'data'),
    prevent_initial_call=True
)
def handle_remove_instance_button_hungary_opt(n_clicks_list, current_selected):
    ...
    global _dropdown_cache_hungary_opt, _comparison_cache_hungary_opt, _graph_cache_hungary_opt
    _dropdown_cache_hungary_opt.clear()
    _comparison_cache_hungary_opt.clear()
    _graph_cache_hungary_opt.clear()
    
    return new_selected


# ==========================================
# GRAPH UPDATE - HUNKIYE OPT
# ==========================================

@callback(
    Output('hun-opt-comparison-graph', 'figure'),
    Input('hun-opt-comparison-selected-instances-store', 'data'),
    prevent_initial_call=False
)
def update_comparison_graph_optimized_hungary_opt(selected_instances):
    """Graph updates"""
    ...
    cache_key = "hun_opt_" + "_".join(map(str, sorted(selected_instances)))
    
    if cache_key in _graph_cache_hungary_opt:
        return _graph_cache_hungary_opt[cache_key]
    ...
    _graph_cache_hungary_opt[cache_key] = fig
    return fig


# ==========================================
# CLIENTSIDE CALLBACKS - HUNKIYE OPT
# ==========================================
...
clientside_callback(
    ...
    function(n_clicks) {
        if (n_clicks > 0) {
            setTimeout(function() {
                const container = document.getElementById('hun-opt-virtual-scroll-container');
                        ...
                        scrollTimeout = setTimeout(function() {
                            if (Math.abs(scrollTop - lastScrollTop) > 3) {
                                ...
                                if (window.dash_clientside && window.dash_clientside.set_props) {
                                    try {
                                        window.dash_clientside.set_props('hun-opt-virtual-scroll-position', {
                                            data: scrollTop
                                        });
                                    } catch (error) {
                                        console.error('HUN OPT scroll error:', error);
                                    }
                                }
                            }
                        }, 30);
    ...
    """,
    Output('hun-opt-virtual-scroll-container', 'data-scroll-initialized'),
    Input('hun-opt-virtual-dropdown-trigger', 'n_clicks'),
    prevent_initial_call=True
)
...
clientside_callback(
        ...
        if (n_clicks > 0 && !window.turOptOutsideClickAttached) {
            setTimeout(function() {
                document.addEventListener('click', function(e) {
                    const dropdown = document.getElementById('hun-opt-virtual-dropdown-trigger');
                    const menu = document.getElementById('hun-opt-virtual-scroll-container');
                    const warning = document.getElementById('hun-opt-warning-overlay');
                    ...
                    if (dropdown && menu && 
                        ...
                        if (menu.style.display === 'block') {
                            ...
                            if (window.dash_clientside && window.dash_clientside.set_props) {
                                window.dash_clientside.set_props('hun-opt-dropdown-open-state', {data: false});
                            }
                        }
                    }
                }, { passive: true });
    ...
    """,
    Output('hun-opt-dummy-outside-click', 'children'),
    Input('hun-opt-virtual-dropdown-trigger', 'n_clicks'),
    prevent_initial_call=True
)

@callback(
    [Output('hun-opt-virtual-item-height', 'data'),
     Output('hun-opt-virtual-container-height', 'data')],
    Input('hun-opt-virtual-dropdown-trigger', 'n_clicks'),
    prevent_initial_call=False
)
def validate_virtual_dimensions_hungary_opt(n_clicks):
    """Validate dimensions"""
    return 40, 320


@callback(
    Output('hun-opt-virtual-scroll-container', 'data-total-items'),
    Input('hun-opt-sort-by-dropdown', 'value'),
    prevent_initial_call=False
)
def validate_total_items_hungary_opt(sort_by):
...
# ==========================================
# TABLE CALLBACKS - HUNKIYE OPT
# ==========================================

@callback(
    [Output("hun-opt-instance-table", "data"),
     Output("hun-opt-table-info", "children"),
     Output("hun-opt-instance-table", "selected_rows")],
    [Input("hun-opt-instance-table", "page_current"),
     Input("hun-opt-table-search-input", "value"),
     Input("hun-opt-table-filters", "data"),
     Input('hun-opt-scatter-plot', 'selectedData'),
     Input('hun-opt-selected-instances-store', 'data'),
     Input('hun-opt-instance-table', 'sort_by')],
    prevent_initial_call=False
)
def update_table_data_hungary_opt(page_current, search_value, filters, selectedData, selected_instances, sort_by):
    """Table data update"""
    ...
    try:
        ...
        print(f"HUN OPT Error in table: {e}")
        import traceback
        traceback.print_exc()
...
@callback(
    [Output("hun-opt-instance-table", "page_current"),
     Output("hun-opt-table-current-page", "children"),
     Output("hun-opt-table-total-pages", "children"),
     Output("hun-opt-table-first-page-btn", "disabled"),
     Output("hun-opt-table-prev-page-btn", "disabled"),
     Output("hun-opt-table-next-page-btn", "disabled"),
     Output("hun-opt-table-last-page-btn", "disabled")],
    [Input("hun-opt-table-first-page-btn", "n_clicks"),
     Input("hun-opt-table-prev-page-btn", "n_clicks"),
     Input("hun-opt-table-next-page-btn", "n_clicks"),
     Input("hun-opt-table-last-page-btn", "n_clicks"),
     Input("hun-opt-table-search-input", "value"),
     Input("hun-opt-table-filters", "data"),
     Input('hun-opt-scatter-plot', 'selectedData'),
     Input('hun-opt-selected-instances-store', 'data')],
    [State("hun-opt-instance-table", "page_current")],
    prevent_initial_call=False
)
def handle_manual_pagination_hungary_opt(first_clicks, prev_clicks, next_clicks, last_clicks,
    ...
    try:
        ...
        if ctx.triggered:
            ...
            if ("hun-opt-table-search-input" in triggered_prop or
                "hun-opt-table-filters" in triggered_prop or
                "hun-opt-scatter-plot.selectedData" in triggered_prop or
                "hun-opt-selected-instances-store" in triggered_prop):
                new_page = 0
            elif "hun-opt-table-first-page-btn" in triggered_prop and first_clicks:
                new_page = 0
            elif "hun-opt-table-prev-page-btn" in triggered_prop and prev_clicks:
                new_page = max(0, current_page - 1)
            elif "hun-opt-table-next-page-btn" in triggered_prop and next_clicks:
                new_page = min(total_pages - 1, current_page + 1)
            elif "hun-opt-table-last-page-btn" in triggered_prop and last_clicks:
        ...
        print(f"HUN OPT pagination error: {e}")
        return 0, "1", "1", True, True, True, True


@callback(
    Output('hun-opt-selected-instances-store', 'data', allow_duplicate=True),
    Input('hun-opt-instance-table', 'selected_rows'),
    [State('hun-opt-instance-table', 'data'),
     State('hun-opt-selected-instances-store', 'data')],
    prevent_initial_call=True
)
def update_selection_from_table_hungary_opt(selected_rows, table_data, current_selected):
...
# ==========================================
# FILTER CHIPS - HUNKIYE OPT
# ==========================================

@callback(
    Output('hun-opt-filter-chips-container', 'children'),
    Input('hun-opt-table-filters', 'data'),
    prevent_initial_call=False
)
def update_filter_chips_hungary_opt(filters):
    ...
    for column, filter_value in filters.items():
        ...
        chip = dbc.Badge(
            [
                chip_text,
                html.Span(" ×",
                    id={"type": "hun-opt-remove-filter", "index": column},
                    style={
                        "marginLeft": "8px",
                        "cursor": "pointer",
                        "fontWeight": "bold",
...
@callback(
    Output('hun-opt-table-filters', 'data'),
    Input({"type": "hun-opt-remove-filter", "index": dash.ALL}, 'n_clicks'),
    State('hun-opt-table-filters', 'data'),
    prevent_initial_call=True
)
def remove_filter_hungary_opt(n_clicks_list, current_filters):
    """Remove filter"""
...
@callback(
    Output('hun-opt-download-instances', 'data'),
    Input('hun-opt-export-selected-btn', 'n_clicks'),
    [State('hun-opt-scatter-plot', 'selectedData'),
     State('hun-opt-selected-instances-store', 'data'),
     State('hun-opt-table-search-input', 'value'),
     State('hun-opt-table-filters', 'data')],
    prevent_initial_call=True
)
def export_selected_instances_hungary_opt(n_clicks, selectedData, selected_instances, search_value, filters):
    ...
    try:
        ...
        filename = f"HUN_OptimizedSolutions_{timestamp}.csv"
        
        return dcc.send_data_frame(export_df.to_csv, filename, index=False)
        
    except Exception as e:
        print(f"HUN OPT export error: {e}")
        return dash.no_update


# ==========================================
# FILTER DROPDOWN - HUNKIYE OPT
# ==========================================

@callback(
    [Output('hun-opt-filter-dropdown-menu', 'style'),
     Output('hun-opt-filter-dropdown-open', 'data'),
     Output('hun-opt-add-filter-trigger-btn', 'className')],
    [Input('hun-opt-add-filter-trigger-btn', 'n_clicks')],
    [State('hun-opt-filter-dropdown-open', 'data')],
    prevent_initial_call=True
)
def toggle_filter_dropdown_hungary_opt(n_clicks, is_open):
...
@callback(
    [Output('hun-opt-filter-step-1', 'style'),
     Output('hun-opt-filter-step-2', 'style'),
     Output('hun-opt-filter-current-step', 'data')],
    [Input({"type": "hun-opt-param-selector", "param": dash.ALL}, 'n_clicks'),
     Input('hun-opt-filter-back-btn', 'n_clicks')],
    [State('hun-opt-filter-current-step', 'data')],
    prevent_initial_call=True
)
def handle_filter_step_navigation_hungary_opt(param_clicks, back_clicks, current_step):
    ...
    if 'hun-opt-filter-back-btn' in triggered_id:
        return (
            {"display": "block"},
            {"display": "none"},
            1
    ...
    elif 'hun-opt-param-selector' in triggered_id:
        return (
            {"display": "none"},
            {"display": "block"},
            2
...
@callback(
    [Output('hun-opt-filter-selected-param', 'data'),
     Output('hun-opt-filter-min-input', 'placeholder'),
     Output('hun-opt-filter-max-input', 'placeholder')],
    [Input({"type": "hun-opt-param-selector", "param": dash.ALL}, 'n_clicks')],
    [State({"type": "hun-opt-param-selector", "param": dash.ALL}, 'id')],
    prevent_initial_call=True
)
def handle_parameter_selection_hungary_opt(n_clicks_list, param_ids):
...
@callback(
    [Output('hun-opt-table-filters', 'data', allow_duplicate=True),
     Output('hun-opt-filter-dropdown-menu', 'style', allow_duplicate=True),
     Output('hun-opt-filter-dropdown-open', 'data', allow_duplicate=True),
     Output('hun-opt-add-filter-trigger-btn', 'className', allow_duplicate=True)],
    [Input('hun-opt-filter-apply-btn', 'n_clicks')],
    [State('hun-opt-filter-selected-param', 'data'),
     State('hun-opt-filter-min-input', 'value'),
     State('hun-opt-filter-max-input', 'value'),
     State('hun-opt-table-filters', 'data')],
    prevent_initial_call=True
)
def apply_filter_from_dropdown_hungary_opt(n_clicks, selected_param, min_val, max_val, current_filters):
    """Apply filter"""
...
# ==========================================
# CLIENTSIDE - WARNING - HUNKIYE OPT
# ==========================================
...
clientside_callback(
    ...
    function(warning_state) {
        if (warning_state && warning_state.show) {
            ...
            window.turOptWarningTimeout = setTimeout(function() {
                if (window.dash_clientside && window.dash_clientside.set_props) {
                    try {
                        window.dash_clientside.set_props('hun-opt-selection-warning-state', {
                            data: {show: false, message: ''}
                        });
                    } catch (error) {
                        console.error('Error auto-clearing HUN OPT warning:', error);
                    }
                }
            }, 3000);
        }
    ...
    """,
    Output('hun-opt-selection-warning-state', 'data', allow_duplicate=True),
    Input('hun-opt-selection-warning-state', 'data'),
    prevent_initial_call=True
)
...
clientside_callback(
    ...
    function(warning_state, dropdown_state) {
        const overlay = document.getElementById('hun-opt-warning-overlay');
    ...
    """,
    Output('hun-opt-warning-overlay', 'data-status', allow_duplicate=True),
    [Input('hun-opt-selection-warning-state', 'data'),
     Input('hun-opt-dropdown-open-state', 'data')],
     prevent_initial_call=True
)
...
clientside_callback(
    ...
    """,
    Output('hun-opt-virtual-dropdown-trigger', 'data-keyboard-handler'),
    Input('hun-opt-virtual-dropdown-trigger', 'n_clicks'),
    prevent_initial_call=True
)
...
clientside_callback(
    ...
    function(n_clicks) {
        if (n_clicks > 0 && !window.turOptFilterDropdownClickListener) {
            setTimeout(function() {
                document.addEventListener('click', function(e) {
                    const trigger = document.getElementById('hun-opt-add-filter-trigger-btn');
                    const menu = document.getElementById('hun-opt-filter-dropdown-menu');
                    ...
                    if (trigger && menu && 
                        ...
                        if (menu.style.display === 'block') {
                            ...
                            if (window.dash_clientside && window.dash_clientside.set_props) {
                                try {
                                    window.dash_clientside.set_props('hun-opt-filter-dropdown-open', {data: false});
                                } catch (error) {
                                    console.error('Error closing HUN OPT filter dropdown:', error);
                                }
                            }
                        }
                    }
    ...
    """,
    Output('hun-opt-add-filter-trigger-btn', 'data-click-listener'),
    Input('hun-opt-add-filter-trigger-btn', 'n_clicks'),
    prevent_initial_call=True
)
...
# ==========================================
# CACHE MANAGEMENT - HUNKIYE OPT
# ==========================================

def clear_caches_hungary_opt():
    """Clear all caches"""
    global _dropdown_cache_hungary_opt, _param_cache_hungary_opt, _comparison_cache_hungary_opt, _graph_cache_hungary_opt
    _dropdown_cache_hungary_opt.clear()
    _param_cache_hungary_opt.clear()
    _comparison_cache_hungary_opt.clear()
    _graph_cache_hungary_opt.clear()
    print("HUN OPT: All caches cleared")

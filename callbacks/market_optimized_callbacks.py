...
# Load the datasets - OPTIMIZED VERSION USES DIFFERENT DATASET
df_plot = pd.read_csv("data/pareto_front_predicted_100_yeni.csv", sep=';')  # Changed to optimized dataset
...
@callback(
    [Output('market-optimized-selected-point-store', 'data'),
     Output('market-optimized-scatter-plot', 'figure')],
    Input('market-optimized-scatter-plot', 'clickData'),
    State('market-optimized-scatter-plot', 'figure'),
...
@callback(
    [Output('market-optimized-spider-chart', 'figure'),
     Output('market-optimized-instances-store', 'data')],
    [Input('market-optimized-selected-point-store', 'data'),
     Input('market-optimized-neighbors-slider', 'value')]
)
...
@callback(
    [Output('market-optimized-instance-table', 'data'),
     Output('market-optimized-table-info', 'children'),
     Output('market-optimized-layers-data', 'data')],
    [Input('market-optimized-instances-store', 'data'),
     Input('market-optimized-selected-point-store', 'data'),
     Input('market-optimized-neighbors-slider', 'value')],
...
@callback(
    Output('market-optimized-download-instances', 'data'),
    Input('export-market-optimized-button', 'n_clicks'),
    [State('market-optimized-instance-table', 'selected_rows'),
     State('market-optimized-instance-table', 'data')],
    prevent_initial_call=True
)
def export_market_selected_instances(n_clicks, selected_rows, table_data):
    ...
    try:
        ...
        print(f"MARKET OPTIMIZED CSV EXPORT: Processing {len(selected_data)} instances for CSV export")
        ...
        print(f"MARKET OPTIMIZED CSV EXPORT: Sending CSV file {filename} with {len(export_df)} rows and {len(export_df.columns)} columns")
        
        # CSV olarak indir
        return dcc.send_data_frame(export_df.to_csv, filename, index=False)
        
    except Exception as e:
        print(f"MARKET OPTIMIZED CSV EXPORT ERROR: {e}")
        import traceback
        traceback.print_exc()
...
@callback(
    Output('market-optimized-instance-table', 'selected_rows'),
    Input('market-optimized-instance-table', 'data'),
    prevent_initial_call=False
)
...
clientside_callback(
    ...
    """,
    Output('market-optimized-hover-state', 'data'),
    Input('market-optimized-layers-data', 'data')
)
...
clientside_callback(
    ...
    """,
    Output('market-optimized-hover-state', 'data', allow_duplicate=True),
    Input('market-optimized-instance-table', 'data'),
    prevent_initial_call=True
)

import dash
import pandas as pd
from utils.hungary_plots import load_all_data_hungary
...
import threading
import re
from pages.hungary_explorer import df
from dash import callback, Output, Input, ctx, html, dcc, State, clientside_callback, callback_context

# ==========================================
# PERFORMANCE OPTIMIZATIONS - HUNKIYE
# ==========================================

# Cache the data - separate cache for Hungary
_cached_data_hungary = None
_cached_df_with_id_hungary = None

def get_cached_data_hungary():
    """Load Hungary data once and cache it"""
    global _cached_data_hungary, _cached_df_with_id_hungary
    if _cached_data_hungary is None:
        _cached_data_hungary = load_all_data_hungary()
        _cached_df_with_id_hungary = _cached_data_hungary.copy()
        _cached_df_with_id_hungary['ID'] = _cached_df_with_id_hungary.index + 1
    return _cached_data_hungary, _cached_df_with_id_hungary

def get_selectable_data_hungary():
    """Get data excluding baseline for user selection"""
    df, df_with_id = get_cached_data_hungary()
    selectable_df = df_with_id[df_with_id.index > 0].copy()
    return selectable_df

# ==========================================
# CACHE DICTIONARIES - HUNKIYE
# ==========================================

_dropdown_cache_hungary = {}
_param_cache_hungary = {}
_comparison_cache_hungary = {}
_graph_cache_hungary = {}

def format_co2_value_hungary(co2_value):
    """Format CO2 value"""
    try:
...
# ==========================================
# VIRTUAL CONTENT UPDATE - HUNKIYE
# ==========================================

@callback(
    [Output('hun-virtual-scroll-container', 'children'),
     Output('hun-virtual-scroll-position', 'data')],
    [Input('hun-virtual-scroll-container', 'scrollTop'),
     Input('hun-comparison-selected-instances-store', 'data'),
     Input('hun-sort-by-dropdown', 'value'),
     Input('hun-sort-order-radio', 'value'),
     Input('hun-virtual-scroll-position', 'data')],
    [State('hun-virtual-item-height', 'data'),
     State('hun-virtual-container-height', 'data')],
    prevent_initial_call=False
)
def update_virtual_content_combined_hungary(scroll_top_prop, selected_instances, sort_by, sort_order, scroll_position_data, item_height, container_height):
    """Virtual scrolling for Hungary explorer"""
    ...
    try:
        ...
        df, df_with_id = get_cached_data_hungary()
        selectable_df = get_selectable_data_hungary()
        ...
        cache_key = f"hun_{scroll_group}_{selected_key}_{sort_by}_{sort_order}"
        
        triggered_id = ctx.triggered_id
        
        if cache_key in _dropdown_cache_hungary and triggered_id != 'hun-selected-instances-store':
            print(f"HUN CACHE HIT: Using cached content for scroll_group={scroll_group}")
            return _dropdown_cache_hungary[cache_key], scroll_top
        ...
        print(f"HUN VIRTUAL RENDER: Scroll={scroll_top}, Range={visible_start}-{visible_end}/{total_items}")
        ...
        for i in range(visible_start, visible_end):
            ...
            unique_checkbox_id = f"hun-virtual-checkbox-{instance_id}-{scroll_group}"
            ...
            visible_items.append(
                html.Div([
                    ...
                    html.Div([
                        dcc.Checklist(
                            id={"type": "hun-virtual-checkbox", "index": instance_id, "unique": unique_checkbox_id},
                ...
                id=f"hun-virtual-item-{instance_id}",
        ...
        print(f"HUN VIRTUAL: Successfully rendered {items_rendered} items")
        
        if len(_dropdown_cache_hungary) > 50:
            old_keys = list(_dropdown_cache_hungary.keys())[:25]
            for key in old_keys:
                del _dropdown_cache_hungary[key]
            print("HUN Cache cleaned")
        
        _dropdown_cache_hungary[cache_key] = visible_items
        
        return visible_items, scroll_top
        
    except Exception as e:
        print(f"HUN Virtual content error: {e}")
        import traceback
        traceback.print_exc()
...
# ==========================================
# WARNING OVERLAY - HUNKIYE
# ==========================================

@callback(
    [Output('hun-warning-overlay', 'children'),
     Output('hun-warning-overlay', 'style')],
    Input('hun-selection-warning-state', 'data'),
    [State('hun-dropdown-open-state', 'data')],
    prevent_initial_call=False
)
def update_warning_overlay_hungary(warning_state, dropdown_is_open):
    """Warning overlay management for Hungary"""
...
@callback(
    Output('hun-warning-overlay', 'style', allow_duplicate=True),
    Input('hun-dropdown-open-state', 'data'),
    [State('hun-selection-warning-state', 'data'),
     State('hun-warning-overlay', 'style')],
    prevent_initial_call=True
)
def update_warning_position_on_dropdown_toggle_hungary(dropdown_is_open, warning_state, current_style):
...
# ==========================================
# SELECTION HANDLING - HUNKIYE
# ==========================================

@callback(
    [Output('hun-comparison-selected-instances-store', 'data'),
     Output('hun-selection-warning-state', 'data')],
    [Input({"type": "hun-virtual-checkbox", "index": dash.ALL, "unique": dash.ALL}, 'value'),
     Input('hun-search-input', 'n_submit')],
    [State({"type": "hun-virtual-checkbox", "index": dash.ALL, "unique": dash.ALL}, 'id'),
     State('hun-comparison-selected-instances-store', 'data'),
     State('hun-search-input', 'value')],
    prevent_initial_call=True
)
def handle_selection_and_search_with_warning_hungary(all_values, n_submit, all_ids, current_selected, search_value):
    """Selection handling for Hungary explorer"""
    ...
    # Handle search input
    if triggered_id == 'hun-search-input' and n_submit and search_value is not None:
        try:
            display_id = int(search_value)
            instance_id = display_id + 1
            
            if 1 <= display_id <= 3889:
                ...
                if instance_id not in new_selected:
                    ...
                    print(f"HUN SEARCH: Added S{display_id} (ID:{instance_id}), total: {len(new_selected)}")
                
                def delayed_cache_clear():
                    time.sleep(0.1)
                    global _dropdown_cache_hungary
                    _dropdown_cache_hungary.clear()
                    print("HUN SEARCH: Cache cleared")
    ...
    if all_values and all_ids:
        try:
            ...
            if 'hun-virtual-checkbox' not in prop_id:
                raise PreventUpdate
            ...
            print(f"HUN SELECTION: Processing change for S{changed_instance_id}")
            ...
            if is_now_checked and changed_instance_id not in new_selected:
                ...
                print(f"HUN SELECTION: Added S{changed_instance_id} (Total: {len(new_selected)})")
                    
            elif not is_now_checked and changed_instance_id in new_selected:
                new_selected.remove(changed_instance_id)
                print(f"HUN SELECTION: Removed S{changed_instance_id} (Total: {len(new_selected)})")
            
            if set(new_selected) != set(current_selected):
                print(f"HUN SELECTION: Updated from {current_selected} to {new_selected}")
                
                def delayed_cache_clear():
                    time.sleep(0.1)
                    global _dropdown_cache_hungary
                    _dropdown_cache_hungary.clear()
                    print("HUN SELECTION: Cache cleared")
...
# ==========================================
# DROPDOWN STATE MANAGEMENT - HUNKIYE
# ==========================================

@callback(
    [Output('hun-virtual-scroll-container', 'style'),
     Output('hun-dropdown-open-state', 'data'),
     Output('hun-virtual-dropdown-trigger', 'children')],
    [Input('hun-virtual-dropdown-trigger', 'n_clicks'),
     Input('hun-comparison-selected-instances-store', 'data')],
    [State('hun-dropdown-open-state', 'data')],
    prevent_initial_call=True
)
def handle_dropdown_state_combined_hungary(n_clicks, selected_instances, is_open):
    """Dropdown toggle for Hungary explorer"""
    
    triggered_id = ctx.triggered_id
    
    if triggered_id == 'hun-virtual-dropdown-trigger' and n_clicks:
        ...
        if new_state:
            global _dropdown_cache_hungary
            _dropdown_cache_hungary.clear()
            print("HUN DROPDOWN: Opened, cleared cache")
        
        trigger_text = get_trigger_text_hungary(selected_instances)
        return style, new_state, trigger_text
    
    if triggered_id == 'hun-selected-instances-store':
        current_style = {
...
def get_trigger_text_hungary(selected_instances):
...
# ==========================================
# SEARCH TOGGLE - HUNKIYE
# ==========================================

@callback(
    [Output('hun-search-input-container', 'className'),
     Output('hun-search-toggle-state', 'data'),
     Output('hun-search-input', 'value'),
     Output('hun-search-toggle-button', 'className')],
    [Input('hun-search-toggle-button', 'n_clicks'),
     Input('hun-search-input', 'n_submit')],
    [State('hun-search-toggle-state', 'data'),
     State('hun-search-input', 'value')],
    prevent_initial_call=True
)
def handle_search_toggle_combined_hungary(toggle_clicks, submit_count, is_open, search_value):
    """Handle search visibility and button styling"""
    
    triggered_id = ctx.triggered_id
    
    if triggered_id == 'hun-search-toggle-button':
    ...
    if triggered_id == 'hun-search-input' and search_value:
...
# ==========================================
# PARAMETER UPDATE - HUNKIYE
# ==========================================

@callback(
    Output('hun-instance-parameters', 'children'),
    Input('hun-comparison-selected-instances-store', 'data'),
    prevent_initial_call=False
)
def update_instance_parameters_optimized_hungary(selected_instances):
    """Parameter updates with caching for Hungary"""
    
    df, _ = get_cached_data_hungary()
    ...
    cache_key = f"hun_{selected_id}_baseline_{baseline_mode}"
    
    if cache_key in _param_cache_hungary:
        return _param_cache_hungary[cache_key]
    ...
    _param_cache_hungary[cache_key] = result
    return result


# ==========================================
# COMPARISON UPDATE - HUNKIYE
# ==========================================

@callback(
    Output('hun-instance-comparison-content', 'children'),
    Input('hun-comparison-selected-instances-store', 'data'),
    prevent_initial_call=False
)
def update_instance_comparison_optimized_hungary(selected_instances):
    """Instance comparison table for Hungary"""
    df, _ = get_cached_data_hungary()
    n = len(df)
    ...
    def format_value(raw_val, metric_key=None):
        try:
            if metric_key == "co2":
                return format_co2_value_hungary(raw_val)
            else:
                if pd.isna(raw_val):
    ...
    def create_instance_content_with_remove_btn(instance_label, instance_id):
        ...
        return html.Div([
            html.Div([
                ...
                html.Button(
                    "×", 
                    id={"type": "hun-remove-instance-btn", "index": instance_id},
...
# ==========================================
# REMOVE BUTTON - HUNKIYE
# ==========================================

@callback(
    Output('hun-comparison-selected-instances-store', 'data', allow_duplicate=True),
    Input({"type": "hun-remove-instance-btn", "index": dash.ALL}, 'n_clicks'),
    State('hun-comparison-selected-instances-store', 'data'),
    prevent_initial_call=True
)
def handle_remove_instance_button_hungary(n_clicks_list, current_selected):
    ...
    try:
        ...
        print(f"HUN REMOVE: Will remove S{instance_id_to_remove}")
    except (json.JSONDecodeError, KeyError, IndexError) as e:
        print(f"HUN Error parsing button prop_id: {prop_id}, error: {e}")
        raise PreventUpdate
    
    new_selected = [inst_id for inst_id in current_selected if inst_id != instance_id_to_remove]
    
    print(f"HUN REMOVE BUTTON: Removed S{instance_id_to_remove}, new selection: {new_selected}")
    
    global _dropdown_cache_hungary, _comparison_cache_hungary, _graph_cache_hungary
    _dropdown_cache_hungary.clear()
    _comparison_cache_hungary.clear()
    _graph_cache_hungary.clear()
    
    return new_selected


# ==========================================
# SYNCHRONIZATION - HUNKIYE
# ==========================================

@callback(
    Output('hun-selected-instances-store', 'data', allow_duplicate=True),
    Input('hun-comparison-selected-instances-store', 'data'),
    prevent_initial_call=True
)
def sync_comparison_to_main_store_hungary(comparison_selected):
    """Sync comparison store to main store"""
    return comparison_selected


# ==========================================
# GRAPH UPDATE - HUNKIYE
# ==========================================

@callback(
    Output('hun-comparison-graph', 'figure'),
    Input('hun-comparison-selected-instances-store', 'data'),
    prevent_initial_call=False
)
def update_comparison_graph_optimized_hungary(selected_instances):
    """Graph updates for Hungary explorer"""
    ...
    # Create cache key
    cache_key = "hun_" + "_".join(map(str, sorted(selected_instances)))
    
    if cache_key in _graph_cache_hungary:
        return _graph_cache_hungary[cache_key]
    ...
    _graph_cache_hungary[cache_key] = fig
    return fig


# ==========================================
# CLIENTSIDE CALLBACKS - HUNKIYE
# ==========================================
...
clientside_callback(
    ...
    function(n_clicks) {
        if (n_clicks > 0) {
            setTimeout(function() {
                const container = document.getElementById('hun-virtual-scroll-container');
                
                if (container && !container.hasScrollListener) {
                    let scrollTimeout;
                    let lastScrollTop = 0;
                    
                    console.log('HUN: Setting up scroll listener...');
                        ...
                        scrollTimeout = setTimeout(function() {
                            if (Math.abs(scrollTop - lastScrollTop) > 3) {
                                ...
                                if (window.dash_clientside && window.dash_clientside.set_props) {
                                    try {
                                        window.dash_clientside.set_props('hun-virtual-scroll-position', {
                                            data: scrollTop
                                        });
                                        console.log(`HUN SCROLL UPDATE: ${scrollTop}`);
                                    } catch (error) {
                                        console.error('HUN scroll error:', error);
                                    }
                                }
                            }
                        }, 30);
                    ...
                    console.log('HUN scroll listener attached');
                }
            }, 100);
        }
        return n_clicks;
    ...
    """,
    Output('hun-virtual-scroll-container', 'data-scroll-initialized'),
    Input('hun-virtual-dropdown-trigger', 'n_clicks'),
    prevent_initial_call=True
)
...
clientside_callback(
        ...
        if (n_clicks > 0 && !window.turOutsideClickAttached) {
            setTimeout(function() {
                document.addEventListener('click', function(e) {
                    const dropdown = document.getElementById('hun-virtual-dropdown-trigger');
                    const menu = document.getElementById('hun-virtual-scroll-container');
                    const warning = document.getElementById('hun-warning-overlay');
                    ...
                    if (dropdown && menu && 
                        ...
                        if (menu.style.display === 'block') {
                            ...
                            if (window.dash_clientside && window.dash_clientside.set_props) {
                                window.dash_clientside.set_props('hun-dropdown-open-state', {data: false});
                            }
                        }
                    }
                }, { passive: true });
    ...
    """,
    Output('hun-dummy-outside-click', 'children'),
    Input('hun-virtual-dropdown-trigger', 'n_clicks'),
    prevent_initial_call=True
)

@callback(
    [Output('hun-virtual-item-height', 'data'),
     Output('hun-virtual-container-height', 'data')],
    Input('hun-virtual-dropdown-trigger', 'n_clicks'),
    prevent_initial_call=False
)
def validate_virtual_dimensions_hungary(n_clicks):
    """Validate virtual scrolling dimensions"""
    return 40, 320


@callback(
    Output('hun-virtual-scroll-container', 'data-total-items'),
    Input('hun-sort-by-dropdown', 'value'),
    prevent_initial_call=False
)
def validate_total_items_hungary(sort_by):
    """Validate total number of items"""
    selectable_df = get_selectable_data_hungary()
    total_items = len(selectable_df)
    print(f"HUN VIRTUAL VALIDATION: Total selectable items: {total_items}")
    return total_items


# ==========================================
# TABLE CALLBACKS - HUNKIYE
# ==========================================

@callback(
    [Output("hun-instance-table", "data"),
     Output("hun-table-info", "children"),
     Output("hun-instance-table", "selected_rows")],
    [Input("hun-instance-table", "page_current"),
     Input("hun-table-search-input", "value"),
     Input("hun-table-filters", "data"),
     Input('hun-scatter-plot', 'selectedData'),
     Input('hun-selected-instances-store', 'data'),
     Input('hun-instance-table', 'sort_by')],
    prevent_initial_call=False
)
def update_table_data_hungary(page_current, search_value, filters, selectedData, selected_instances, sort_by):
    """Table data update for Hungary"""
    ...
    try:
        ...
        print(f"HUN TABLE: Returning {len(table_data)} rows, total_rows={total_rows}")
        return table_data, table_info, selected_rows_on_page
        
    except Exception as e:
        print(f"HUN Error in update_table_data: {e}")
        import traceback
        traceback.print_exc()
...
@callback(
    Output('hun-selected-instances-store', 'data', allow_duplicate=True),
    Input('hun-instance-table', 'selected_rows'),
    [State('hun-instance-table', 'data'),
     State('hun-selected-instances-store', 'data')],
    prevent_initial_call=True
)
def update_selection_from_table_hungary(selected_rows, table_data, current_selected):
...
# ==========================================
# FILTER CHIPS - HUNKIYE
# ==========================================

@callback(
    Output('hun-filter-chips-container', 'children'),
    Input('hun-table-filters', 'data'),
    prevent_initial_call=False
)
def update_filter_chips_hungary(filters):
    ...
    for column, filter_value in filters.items():
        ...
        chip = dbc.Badge(
            [
                chip_text,
                html.Span(" ×",
                    id={"type": "hun-remove-filter", "index": column},
                    style={
                        "marginLeft": "8px",
                        "cursor": "pointer",
                        "fontWeight": "bold",
...
@callback(
    Output('hun-table-filters', 'data'),
    Input({"type": "hun-remove-filter", "index": dash.ALL}, 'n_clicks'),
    State('hun-table-filters', 'data'),
    prevent_initial_call=True
)
def remove_filter_hungary(n_clicks_list, current_filters):
...
@callback(
    Output('hun-download-instances', 'data'),
    Input('hun-export-selected-btn', 'n_clicks'),
    [State('hun-scatter-plot', 'selectedData'),
     State('hun-selected-instances-store', 'data'),
     State('hun-table-search-input', 'value'),
     State('hun-table-filters', 'data')],
    prevent_initial_call=True
)
def export_selected_instances_hungary(n_clicks, selectedData, selected_instances, search_value, filters):
    ...
    try:
        ...
        filename = f"HUN_AllSolutions_{timestamp}.csv"
        
        print(f"HUN EXPORT: Exporting {len(export_df)} instances")
        
        return dcc.send_data_frame(export_df.to_csv, filename, index=False)
        
    except Exception as e:
        print(f"HUN Export error: {e}")
        import traceback
        traceback.print_exc()
        return dash.no_update
...
@callback(
    [Output("hun-instance-table", "page_current"),
     Output("hun-table-current-page", "children"),
     Output("hun-table-total-pages", "children"),
     Output("hun-table-first-page-btn", "disabled"),
     Output("hun-table-prev-page-btn", "disabled"),
     Output("hun-table-next-page-btn", "disabled"),
     Output("hun-table-last-page-btn", "disabled")],
    [Input("hun-table-first-page-btn", "n_clicks"),
     Input("hun-table-prev-page-btn", "n_clicks"),
     Input("hun-table-next-page-btn", "n_clicks"),
     Input("hun-table-last-page-btn", "n_clicks"),
     Input("hun-table-search-input", "value"),
     Input("hun-table-filters", "data"),
     Input('hun-scatter-plot', 'selectedData'),
     Input('hun-selected-instances-store', 'data')],
    [State("hun-instance-table", "page_current")],
    prevent_initial_call=False
)
def handle_manual_pagination_updated_hungary(first_clicks, prev_clicks, next_clicks, last_clicks,
    ...
    try:
        ...
        if ctx.triggered:
            ...
            if ("hun-table-search-input" in triggered_prop or
                "hun-table-filters" in triggered_prop or
                "hun-scatter-plot.selectedData" in triggered_prop or
                "hun-selected-instances-store" in triggered_prop):
                new_page = 0
            elif "hun-table-first-page-btn" in triggered_prop and first_clicks:
                new_page = 0
            elif "hun-table-prev-page-btn" in triggered_prop and prev_clicks:
                new_page = max(0, current_page - 1)
            elif "hun-table-next-page-btn" in triggered_prop and next_clicks:
                new_page = min(total_pages - 1, current_page + 1)
            elif "hun-table-last-page-btn" in triggered_prop and last_clicks:
        ...
        print(f"HUN Error in pagination: {e}")
        import traceback
        traceback.print_exc()
...
# ==========================================
# FILTER DROPDOWN - HUNKIYE
# ==========================================

@callback(
    [Output('hun-filter-dropdown-menu', 'style'),
     Output('hun-filter-dropdown-open', 'data'),
     Output('hun-add-filter-trigger-btn', 'className')],
    [Input('hun-add-filter-trigger-btn', 'n_clicks')],
    [State('hun-filter-dropdown-open', 'data')],
    prevent_initial_call=True
)
def toggle_filter_dropdown_hungary(n_clicks, is_open):
...
@callback(
    [Output('hun-filter-step-1', 'style'),
     Output('hun-filter-step-2', 'style'),
     Output('hun-filter-current-step', 'data')],
    [Input({"type": "hun-param-selector", "param": dash.ALL}, 'n_clicks'),
     Input('hun-filter-back-btn', 'n_clicks')],
    [State('hun-filter-current-step', 'data')],
    prevent_initial_call=True
)
def handle_filter_step_navigation_hungary(param_clicks, back_clicks, current_step):
    ...
    if 'hun-filter-back-btn' in triggered_id:
        return (
            {"display": "block"},
            {"display": "none"},
            1
    ...
    elif 'hun-param-selector' in triggered_id:
        return (
            {"display": "none"},
            {"display": "block"},
            2
...
@callback(
    [Output('hun-filter-selected-param', 'data'),
     Output('hun-filter-min-input', 'placeholder'),
     Output('hun-filter-max-input', 'placeholder')],
    [Input({"type": "hun-param-selector", "param": dash.ALL}, 'n_clicks')],
    [State({"type": "hun-param-selector", "param": dash.ALL}, 'id')],
    prevent_initial_call=True
)
def handle_parameter_selection_hungary(n_clicks_list, param_ids):
...
@callback(
    [Output('hun-table-filters', 'data', allow_duplicate=True),
     Output('hun-filter-dropdown-menu', 'style', allow_duplicate=True),
     Output('hun-filter-dropdown-open', 'data', allow_duplicate=True),
     Output('hun-add-filter-trigger-btn', 'className', allow_duplicate=True)],
    [Input('hun-filter-apply-btn', 'n_clicks')],
    [State('hun-filter-selected-param', 'data'),
     State('hun-filter-min-input', 'value'),
     State('hun-filter-max-input', 'value'),
     State('hun-table-filters', 'data')],
    prevent_initial_call=True
)
def apply_filter_from_dropdown_hungary(n_clicks, selected_param, min_val, max_val, current_filters):
...
# ==========================================
# CLIENTSIDE - WARNING AUTO-CLEAR - HUNKIYE
# ==========================================
...
clientside_callback(
    ...
    function(warning_state) {
        if (warning_state && warning_state.show) {
            console.log('HUN Warning shown, will auto-clear in 3 seconds');
            ...
            window.turWarningTimeout = setTimeout(function() {
                console.log('Auto-clearing HUN warning...');
                
                if (window.dash_clientside && window.dash_clientside.set_props) {
                    try {
                        window.dash_clientside.set_props('hun-selection-warning-state', {
                            data: {show: false, message: ''}
                        });
                    } catch (error) {
                        console.error('Error auto-clearing HUN warning:', error);
                    }
                }
            }, 3000);
        }
    ...
    """,
    Output('hun-selection-warning-state', 'data', allow_duplicate=True),
    Input('hun-selection-warning-state', 'data'),
    prevent_initial_call=True
)
...
clientside_callback(
    ...
    function(n_clicks, warning_state) {
        if (n_clicks > 0 && warning_state && warning_state.show) {
            console.log('HUN Dropdown opened, clearing warning');
...
clientside_callback(
    ...
    function(warning_state, dropdown_state) {
        const overlay = document.getElementById('hun-warning-overlay');
        ...
        if (overlay) {
            ...
            if (warning_state && warning_state.show) {
                ...
                console.log('HUN Warning overlay shown');
            } else {
                ...
                console.log('HUN Warning overlay hidden');
            }
        }
    ...
    """,
    Output('hun-warning-overlay', 'data-status', allow_duplicate=True),
    [Input('hun-selection-warning-state', 'data'),
     Input('hun-dropdown-open-state', 'data')],
     prevent_initial_call=True
)
...
clientside_callback(
    ...
    """,
    Output('hun-virtual-dropdown-trigger', 'data-keyboard-handler'),
    Input('hun-virtual-dropdown-trigger', 'n_clicks'),
    prevent_initial_call=True
)
...
clientside_callback(
    ...
    function(n_clicks) {
        if (n_clicks > 0 && !window.turFilterDropdownClickListener) {
            setTimeout(function() {
                document.addEventListener('click', function(e) {
                    const trigger = document.getElementById('hun-add-filter-trigger-btn');
                    const menu = document.getElementById('hun-filter-dropdown-menu');
                    ...
                    if (trigger && menu && 
                        ...
                        if (menu.style.display === 'block') {
                            ...
                            if (window.dash_clientside && window.dash_clientside.set_props) {
                                try {
                                    window.dash_clientside.set_props('hun-filter-dropdown-open', {data: false});
                                } catch (error) {
                                    console.error('Error closing HUN filter dropdown:', error);
                                }
                            }
                        }
                    }
    ...
    """,
    Output('hun-add-filter-trigger-btn', 'data-click-listener'),
    Input('hun-add-filter-trigger-btn', 'n_clicks'),
    prevent_initial_call=True
)
...
# ==========================================
# CACHE MANAGEMENT - HUNKIYE
# ==========================================

def clear_caches_hungary():
    """Clear all Hungary caches"""
    global _dropdown_cache_hungary, _param_cache_hungary, _comparison_cache_hungary, _graph_cache_hungary
    _dropdown_cache_hungary.clear()
    _param_cache_hungary.clear()
    _comparison_cache_hungary.clear()
    _graph_cache_hungary.clear()
    print("HUN: All caches cleared")

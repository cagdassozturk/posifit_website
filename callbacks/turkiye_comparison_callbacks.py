import dash
import pandas as pd
from utils.turkiye_plots import load_all_data_turkiye
...
import threading
import re
from pages.turkiye_explorer import df
from dash import callback, Output, Input, ctx, html, dcc, State, clientside_callback, callback_context

# ==========================================
# PERFORMANCE OPTIMIZATIONS - TURKIYE
# ==========================================

# Cache the data - separate cache for Turkiye
_cached_data_turkiye = None
_cached_df_with_id_turkiye = None

def get_cached_data_turkiye():
    """Load Turkiye data once and cache it"""
    global _cached_data_turkiye, _cached_df_with_id_turkiye
    if _cached_data_turkiye is None:
        _cached_data_turkiye = load_all_data_turkiye()
        _cached_df_with_id_turkiye = _cached_data_turkiye.copy()
        _cached_df_with_id_turkiye['ID'] = _cached_df_with_id_turkiye.index + 1
    return _cached_data_turkiye, _cached_df_with_id_turkiye

def get_selectable_data_turkiye():
    """Get data excluding baseline for user selection"""
    df, df_with_id = get_cached_data_turkiye()
    selectable_df = df_with_id[df_with_id.index > 0].copy()
    return selectable_df

# ==========================================
# CACHE DICTIONARIES - TURKIYE
# ==========================================

_dropdown_cache_turkiye = {}
_param_cache_turkiye = {}
_comparison_cache_turkiye = {}
_graph_cache_turkiye = {}

def format_co2_value_turkiye(co2_value):
    """Format CO2 value"""
    try:
...
# ==========================================
# VIRTUAL CONTENT UPDATE - TURKIYE
# ==========================================

@callback(
    [Output('tur-virtual-scroll-container', 'children'),
     Output('tur-virtual-scroll-position', 'data')],
    [Input('tur-virtual-scroll-container', 'scrollTop'),
     Input('tur-comparison-selected-instances-store', 'data'),
     Input('tur-sort-by-dropdown', 'value'),
     Input('tur-sort-order-radio', 'value'),
     Input('tur-virtual-scroll-position', 'data')],
    [State('tur-virtual-item-height', 'data'),
     State('tur-virtual-container-height', 'data')],
    prevent_initial_call=False
)
def update_virtual_content_combined_turkiye(scroll_top_prop, selected_instances, sort_by, sort_order, scroll_position_data, item_height, container_height):
    """Virtual scrolling for Turkiye explorer"""
    ...
    try:
        ...
        df, df_with_id = get_cached_data_turkiye()
        selectable_df = get_selectable_data_turkiye()
        ...
        cache_key = f"tur_{scroll_group}_{selected_key}_{sort_by}_{sort_order}"
        
        triggered_id = ctx.triggered_id
        
        if cache_key in _dropdown_cache_turkiye and triggered_id != 'tur-selected-instances-store':
            print(f"TUR CACHE HIT: Using cached content for scroll_group={scroll_group}")
            return _dropdown_cache_turkiye[cache_key], scroll_top
        ...
        print(f"TUR VIRTUAL RENDER: Scroll={scroll_top}, Range={visible_start}-{visible_end}/{total_items}")
        ...
        for i in range(visible_start, visible_end):
            ...
            unique_checkbox_id = f"tur-virtual-checkbox-{instance_id}-{scroll_group}"
            ...
            visible_items.append(
                html.Div([
                    ...
                    html.Div([
                        dcc.Checklist(
                            id={"type": "tur-virtual-checkbox", "index": instance_id, "unique": unique_checkbox_id},
                ...
                id=f"tur-virtual-item-{instance_id}",
        ...
        print(f"TUR VIRTUAL: Successfully rendered {items_rendered} items")
        
        if len(_dropdown_cache_turkiye) > 50:
            old_keys = list(_dropdown_cache_turkiye.keys())[:25]
            for key in old_keys:
                del _dropdown_cache_turkiye[key]
            print("TUR Cache cleaned")
        
        _dropdown_cache_turkiye[cache_key] = visible_items
        
        return visible_items, scroll_top
        
    except Exception as e:
        print(f"TUR Virtual content error: {e}")
        import traceback
        traceback.print_exc()
...
# ==========================================
# WARNING OVERLAY - TURKIYE
# ==========================================

@callback(
    [Output('tur-warning-overlay', 'children'),
     Output('tur-warning-overlay', 'style')],
    Input('tur-selection-warning-state', 'data'),
    [State('tur-dropdown-open-state', 'data')],
    prevent_initial_call=False
)
def update_warning_overlay_turkiye(warning_state, dropdown_is_open):
    """Warning overlay management for Turkiye"""
...
@callback(
    Output('tur-warning-overlay', 'style', allow_duplicate=True),
    Input('tur-dropdown-open-state', 'data'),
    [State('tur-selection-warning-state', 'data'),
     State('tur-warning-overlay', 'style')],
    prevent_initial_call=True
)
def update_warning_position_on_dropdown_toggle_turkiye(dropdown_is_open, warning_state, current_style):
...
# ==========================================
# SELECTION HANDLING - TURKIYE
# ==========================================

@callback(
    [Output('tur-comparison-selected-instances-store', 'data'),
     Output('tur-selection-warning-state', 'data')],
    [Input({"type": "tur-virtual-checkbox", "index": dash.ALL, "unique": dash.ALL}, 'value'),
     Input('tur-search-input', 'n_submit')],
    [State({"type": "tur-virtual-checkbox", "index": dash.ALL, "unique": dash.ALL}, 'id'),
     State('tur-comparison-selected-instances-store', 'data'),
     State('tur-search-input', 'value')],
    prevent_initial_call=True
)
def handle_selection_and_search_with_warning_turkiye(all_values, n_submit, all_ids, current_selected, search_value):
    """Selection handling for Turkiye explorer"""
    ...
    # Handle search input
    if triggered_id == 'tur-search-input' and n_submit and search_value is not None:
        try:
            ...
            if 1 <= display_id <= 4320:
                ...
                if instance_id not in new_selected:
                    ...
                    print(f"TUR SEARCH: Added S{display_id} (ID:{instance_id}), total: {len(new_selected)}")
                
                def delayed_cache_clear():
                    time.sleep(0.1)
                    global _dropdown_cache_turkiye
                    _dropdown_cache_turkiye.clear()
                    print("TUR SEARCH: Cache cleared")
    ...
    if all_values and all_ids:
        try:
            ...
            if 'tur-virtual-checkbox' not in prop_id:
                raise PreventUpdate
            ...
            print(f"TUR SELECTION: Processing change for S{changed_instance_id}")
            ...
            if is_now_checked and changed_instance_id not in new_selected:
                ...
                print(f"TUR SELECTION: Added S{changed_instance_id} (Total: {len(new_selected)})")
                    
            elif not is_now_checked and changed_instance_id in new_selected:
                new_selected.remove(changed_instance_id)
                print(f"TUR SELECTION: Removed S{changed_instance_id} (Total: {len(new_selected)})")
            
            if set(new_selected) != set(current_selected):
                print(f"TUR SELECTION: Updated from {current_selected} to {new_selected}")
                
                def delayed_cache_clear():
                    time.sleep(0.1)
                    global _dropdown_cache_turkiye
                    _dropdown_cache_turkiye.clear()
                    print("TUR SELECTION: Cache cleared")
...
# ==========================================
# DROPDOWN STATE MANAGEMENT - TURKIYE
# ==========================================

@callback(
    [Output('tur-virtual-scroll-container', 'style'),
     Output('tur-dropdown-open-state', 'data'),
     Output('tur-virtual-dropdown-trigger', 'children')],
    [Input('tur-virtual-dropdown-trigger', 'n_clicks'),
     Input('tur-comparison-selected-instances-store', 'data')],
    [State('tur-dropdown-open-state', 'data')],
    prevent_initial_call=True
)
def handle_dropdown_state_combined_turkiye(n_clicks, selected_instances, is_open):
    """Dropdown toggle for Turkiye explorer"""
    
    triggered_id = ctx.triggered_id
    
    if triggered_id == 'tur-virtual-dropdown-trigger' and n_clicks:
        ...
        if new_state:
            global _dropdown_cache_turkiye
            _dropdown_cache_turkiye.clear()
            print("TUR DROPDOWN: Opened, cleared cache")
        
        trigger_text = get_trigger_text_turkiye(selected_instances)
        return style, new_state, trigger_text
    
    if triggered_id == 'tur-selected-instances-store':
        current_style = {
...
def get_trigger_text_turkiye(selected_instances):
...
# ==========================================
# SEARCH TOGGLE - TURKIYE
# ==========================================

@callback(
    [Output('tur-search-input-container', 'className'),
     Output('tur-search-toggle-state', 'data'),
     Output('tur-search-input', 'value'),
     Output('tur-search-toggle-button', 'className')],
    [Input('tur-search-toggle-button', 'n_clicks'),
     Input('tur-search-input', 'n_submit')],
    [State('tur-search-toggle-state', 'data'),
     State('tur-search-input', 'value')],
    prevent_initial_call=True
)
def handle_search_toggle_combined_turkiye(toggle_clicks, submit_count, is_open, search_value):
    """Handle search visibility and button styling"""
    
    triggered_id = ctx.triggered_id
    
    if triggered_id == 'tur-search-toggle-button':
    ...
    if triggered_id == 'tur-search-input' and search_value:
...
# ==========================================
# PARAMETER UPDATE - TURKIYE
# ==========================================

@callback(
    Output('tur-instance-parameters', 'children'),
    Input('tur-comparison-selected-instances-store', 'data'),
    prevent_initial_call=False
)
def update_instance_parameters_optimized_turkiye(selected_instances):
    """Parameter updates with caching for Turkiye"""
    
    df, _ = get_cached_data_turkiye()
    ...
    cache_key = f"tur_{selected_id}_baseline_{baseline_mode}"
    
    if cache_key in _param_cache_turkiye:
        return _param_cache_turkiye[cache_key]
    ...
    _param_cache_turkiye[cache_key] = result
    return result


# ==========================================
# COMPARISON UPDATE - TURKIYE
# ==========================================

@callback(
    Output('tur-instance-comparison-content', 'children'),
    Input('tur-comparison-selected-instances-store', 'data'),
    prevent_initial_call=False
)
def update_instance_comparison_optimized_turkiye(selected_instances):
    """Instance comparison table for Turkiye"""
    df, _ = get_cached_data_turkiye()
    n = len(df)
    ...
    def format_value(raw_val, metric_key=None):
        try:
            if metric_key == "co2":
                return format_co2_value_turkiye(raw_val)
            else:
                if pd.isna(raw_val):
    ...
    def create_instance_content_with_remove_btn(instance_label, instance_id):
        ...
        return html.Div([
            html.Div([
                ...
                html.Button(
                    "×", 
                    id={"type": "tur-remove-instance-btn", "index": instance_id},
...
# ==========================================
# REMOVE BUTTON - TURKIYE
# ==========================================

@callback(
    Output('tur-comparison-selected-instances-store', 'data', allow_duplicate=True),
    Input({"type": "tur-remove-instance-btn", "index": dash.ALL}, 'n_clicks'),
    State('tur-comparison-selected-instances-store', 'data'),
    prevent_initial_call=True
)
def handle_remove_instance_button_turkiye(n_clicks_list, current_selected):
    ...
    try:
        ...
        print(f"TUR REMOVE: Will remove S{instance_id_to_remove}")
    except (json.JSONDecodeError, KeyError, IndexError) as e:
        print(f"TUR Error parsing button prop_id: {prop_id}, error: {e}")
        raise PreventUpdate
    
    new_selected = [inst_id for inst_id in current_selected if inst_id != instance_id_to_remove]
    
    print(f"TUR REMOVE BUTTON: Removed S{instance_id_to_remove}, new selection: {new_selected}")
    
    global _dropdown_cache_turkiye, _comparison_cache_turkiye, _graph_cache_turkiye
    _dropdown_cache_turkiye.clear()
    _comparison_cache_turkiye.clear()
    _graph_cache_turkiye.clear()
    
    return new_selected


# ==========================================
# SYNCHRONIZATION - TURKIYE
# ==========================================

@callback(
    Output('tur-selected-instances-store', 'data', allow_duplicate=True),
    Input('tur-comparison-selected-instances-store', 'data'),
    prevent_initial_call=True
)
def sync_comparison_to_main_store_turkiye(comparison_selected):
    """Sync comparison store to main store"""
    return comparison_selected


# ==========================================
# GRAPH UPDATE - TURKIYE
# ==========================================

@callback(
    Output('tur-comparison-graph', 'figure'),
    Input('tur-comparison-selected-instances-store', 'data'),
    prevent_initial_call=False
)
def update_comparison_graph_optimized_turkiye(selected_instances):
    """Graph updates for Turkiye explorer"""
    ...
    # Create cache key
    cache_key = "tur_" + "_".join(map(str, sorted(selected_instances)))
    
    if cache_key in _graph_cache_turkiye:
        return _graph_cache_turkiye[cache_key]
    ...
    _graph_cache_turkiye[cache_key] = fig
    return fig


# ==========================================
# CLIENTSIDE CALLBACKS - TURKIYE
# ==========================================
...
clientside_callback(
    ...
    function(n_clicks) {
        if (n_clicks > 0) {
            setTimeout(function() {
                const container = document.getElementById('tur-virtual-scroll-container');
                
                if (container && !container.hasScrollListener) {
                    let scrollTimeout;
                    let lastScrollTop = 0;
                    
                    console.log('TUR: Setting up scroll listener...');
                        ...
                        scrollTimeout = setTimeout(function() {
                            if (Math.abs(scrollTop - lastScrollTop) > 3) {
                                ...
                                if (window.dash_clientside && window.dash_clientside.set_props) {
                                    try {
                                        window.dash_clientside.set_props('tur-virtual-scroll-position', {
                                            data: scrollTop
                                        });
                                        console.log(`TUR SCROLL UPDATE: ${scrollTop}`);
                                    } catch (error) {
                                        console.error('TUR scroll error:', error);
                                    }
                                }
                            }
                        }, 30);
                    ...
                    console.log('TUR scroll listener attached');
                }
            }, 100);
        }
        return n_clicks;
    ...
    """,
    Output('tur-virtual-scroll-container', 'data-scroll-initialized'),
    Input('tur-virtual-dropdown-trigger', 'n_clicks'),
    prevent_initial_call=True
)
...
clientside_callback(
        ...
        if (n_clicks > 0 && !window.turOutsideClickAttached) {
            setTimeout(function() {
                document.addEventListener('click', function(e) {
                    const dropdown = document.getElementById('tur-virtual-dropdown-trigger');
                    const menu = document.getElementById('tur-virtual-scroll-container');
                    const warning = document.getElementById('tur-warning-overlay');
                    ...
                    if (dropdown && menu && 
                        ...
                        if (menu.style.display === 'block') {
                            ...
                            if (window.dash_clientside && window.dash_clientside.set_props) {
                                window.dash_clientside.set_props('tur-dropdown-open-state', {data: false});
                            }
                        }
                    }
                }, { passive: true });
    ...
    """,
    Output('tur-dummy-outside-click', 'children'),
    Input('tur-virtual-dropdown-trigger', 'n_clicks'),
    prevent_initial_call=True
)

@callback(
    [Output('tur-virtual-item-height', 'data'),
     Output('tur-virtual-container-height', 'data')],
    Input('tur-virtual-dropdown-trigger', 'n_clicks'),
    prevent_initial_call=False
)
def validate_virtual_dimensions_turkiye(n_clicks):
    """Validate virtual scrolling dimensions"""
    return 40, 320


@callback(
    Output('tur-virtual-scroll-container', 'data-total-items'),
    Input('tur-sort-by-dropdown', 'value'),
    prevent_initial_call=False
)
def validate_total_items_turkiye(sort_by):
    """Validate total number of items"""
    selectable_df = get_selectable_data_turkiye()
    total_items = len(selectable_df)
    print(f"TUR VIRTUAL VALIDATION: Total selectable items: {total_items}")
    return total_items


# ==========================================
# TABLE CALLBACKS - TURKIYE
# ==========================================

@callback(
    [Output("tur-instance-table", "data"),
     Output("tur-table-info", "children"),
     Output("tur-instance-table", "selected_rows")],
    [Input("tur-instance-table", "page_current"),
     Input("tur-table-search-input", "value"),
     Input("tur-table-filters", "data"),
     Input('tur-scatter-plot', 'selectedData'),
     Input('tur-selected-instances-store', 'data'),
     Input('tur-instance-table', 'sort_by')],
    prevent_initial_call=False
)
def update_table_data_turkiye(page_current, search_value, filters, selectedData, selected_instances, sort_by):
    """Table data update for Turkiye"""
    ...
    try:
        ...
        print(f"TUR TABLE: Returning {len(table_data)} rows, total_rows={total_rows}")
        return table_data, table_info, selected_rows_on_page
        
    except Exception as e:
        print(f"TUR Error in update_table_data: {e}")
        import traceback
        traceback.print_exc()
...
@callback(
    Output('tur-selected-instances-store', 'data', allow_duplicate=True),
    Input('tur-instance-table', 'selected_rows'),
    [State('tur-instance-table', 'data'),
     State('tur-selected-instances-store', 'data')],
    prevent_initial_call=True
)
def update_selection_from_table_turkiye(selected_rows, table_data, current_selected):
...
# ==========================================
# FILTER CHIPS - TURKIYE
# ==========================================

@callback(
    Output('tur-filter-chips-container', 'children'),
    Input('tur-table-filters', 'data'),
    prevent_initial_call=False
)
def update_filter_chips_turkiye(filters):
    ...
    for column, filter_value in filters.items():
        ...
        chip = dbc.Badge(
            [
                chip_text,
                html.Span(" ×",
                    id={"type": "tur-remove-filter", "index": column},
                    style={
                        "marginLeft": "8px",
                        "cursor": "pointer",
                        "fontWeight": "bold",
...
@callback(
    Output('tur-table-filters', 'data'),
    Input({"type": "tur-remove-filter", "index": dash.ALL}, 'n_clicks'),
    State('tur-table-filters', 'data'),
    prevent_initial_call=True
)
def remove_filter_turkiye(n_clicks_list, current_filters):
...
@callback(
    Output('tur-download-instances', 'data'),
    Input('tur-export-selected-btn', 'n_clicks'),
    [State('tur-scatter-plot', 'selectedData'),
     State('tur-selected-instances-store', 'data'),
     State('tur-table-search-input', 'value'),
     State('tur-table-filters', 'data')],
    prevent_initial_call=True
)
def export_selected_instances_turkiye(n_clicks, selectedData, selected_instances, search_value, filters):
    ...
    try:
        ...
        filename = f"TUR_AllSolutions_{timestamp}.csv"
        
        print(f"TUR EXPORT: Exporting {len(export_df)} instances")
        
        return dcc.send_data_frame(export_df.to_csv, filename, index=False)
        
    except Exception as e:
        print(f"TUR Export error: {e}")
        import traceback
        traceback.print_exc()
        return dash.no_update
...
@callback(
    [Output("tur-instance-table", "page_current"),
     Output("tur-table-current-page", "children"),
     Output("tur-table-total-pages", "children"),
     Output("tur-table-first-page-btn", "disabled"),
     Output("tur-table-prev-page-btn", "disabled"),
     Output("tur-table-next-page-btn", "disabled"),
     Output("tur-table-last-page-btn", "disabled")],
    [Input("tur-table-first-page-btn", "n_clicks"),
     Input("tur-table-prev-page-btn", "n_clicks"),
     Input("tur-table-next-page-btn", "n_clicks"),
     Input("tur-table-last-page-btn", "n_clicks"),
     Input("tur-table-search-input", "value"),
     Input("tur-table-filters", "data"),
     Input('tur-scatter-plot', 'selectedData'),
     Input('tur-selected-instances-store', 'data')],
    [State("tur-instance-table", "page_current")],
    prevent_initial_call=False
)
def handle_manual_pagination_updated_turkiye(first_clicks, prev_clicks, next_clicks, last_clicks,
    ...
    try:
        ...
        if ctx.triggered:
            ...
            if ("tur-table-search-input" in triggered_prop or
                "tur-table-filters" in triggered_prop or
                "tur-scatter-plot.selectedData" in triggered_prop or
                "tur-selected-instances-store" in triggered_prop):
                new_page = 0
            elif "tur-table-first-page-btn" in triggered_prop and first_clicks:
                new_page = 0
            elif "tur-table-prev-page-btn" in triggered_prop and prev_clicks:
                new_page = max(0, current_page - 1)
            elif "tur-table-next-page-btn" in triggered_prop and next_clicks:
                new_page = min(total_pages - 1, current_page + 1)
            elif "tur-table-last-page-btn" in triggered_prop and last_clicks:
        ...
        print(f"TUR Error in pagination: {e}")
        import traceback
        traceback.print_exc()
...
# ==========================================
# FILTER DROPDOWN - TURKIYE
# ==========================================

@callback(
    [Output('tur-filter-dropdown-menu', 'style'),
     Output('tur-filter-dropdown-open', 'data'),
     Output('tur-add-filter-trigger-btn', 'className')],
    [Input('tur-add-filter-trigger-btn', 'n_clicks')],
    [State('tur-filter-dropdown-open', 'data')],
    prevent_initial_call=True
)
def toggle_filter_dropdown_turkiye(n_clicks, is_open):
...
@callback(
    [Output('tur-filter-step-1', 'style'),
     Output('tur-filter-step-2', 'style'),
     Output('tur-filter-current-step', 'data')],
    [Input({"type": "tur-param-selector", "param": dash.ALL}, 'n_clicks'),
     Input('tur-filter-back-btn', 'n_clicks')],
    [State('tur-filter-current-step', 'data')],
    prevent_initial_call=True
)
def handle_filter_step_navigation_turkiye(param_clicks, back_clicks, current_step):
    ...
    if 'tur-filter-back-btn' in triggered_id:
        return (
            {"display": "block"},
            {"display": "none"},
            1
    ...
    elif 'tur-param-selector' in triggered_id:
        return (
            {"display": "none"},
            {"display": "block"},
            2
...
@callback(
    [Output('tur-filter-selected-param', 'data'),
     Output('tur-filter-min-input', 'placeholder'),
     Output('tur-filter-max-input', 'placeholder')],
    [Input({"type": "tur-param-selector", "param": dash.ALL}, 'n_clicks')],
    [State({"type": "tur-param-selector", "param": dash.ALL}, 'id')],
    prevent_initial_call=True
)
def handle_parameter_selection_turkiye(n_clicks_list, param_ids):
...
@callback(
    [Output('tur-table-filters', 'data', allow_duplicate=True),
     Output('tur-filter-dropdown-menu', 'style', allow_duplicate=True),
     Output('tur-filter-dropdown-open', 'data', allow_duplicate=True),
     Output('tur-add-filter-trigger-btn', 'className', allow_duplicate=True)],
    [Input('tur-filter-apply-btn', 'n_clicks')],
    [State('tur-filter-selected-param', 'data'),
     State('tur-filter-min-input', 'value'),
     State('tur-filter-max-input', 'value'),
     State('tur-table-filters', 'data')],
    prevent_initial_call=True
)
def apply_filter_from_dropdown_turkiye(n_clicks, selected_param, min_val, max_val, current_filters):
...
# ==========================================
# CLIENTSIDE - WARNING AUTO-CLEAR - TURKIYE
# ==========================================
...
clientside_callback(
    ...
    function(warning_state) {
        if (warning_state && warning_state.show) {
            console.log('TUR Warning shown, will auto-clear in 3 seconds');
            ...
            window.turWarningTimeout = setTimeout(function() {
                console.log('Auto-clearing TUR warning...');
                
                if (window.dash_clientside && window.dash_clientside.set_props) {
                    try {
                        window.dash_clientside.set_props('tur-selection-warning-state', {
                            data: {show: false, message: ''}
                        });
                    } catch (error) {
                        console.error('Error auto-clearing TUR warning:', error);
                    }
                }
            }, 3000);
        }
    ...
    """,
    Output('tur-selection-warning-state', 'data', allow_duplicate=True),
    Input('tur-selection-warning-state', 'data'),
    prevent_initial_call=True
)
...
clientside_callback(
    ...
    function(n_clicks, warning_state) {
        if (n_clicks > 0 && warning_state && warning_state.show) {
            console.log('TUR Dropdown opened, clearing warning');
...
clientside_callback(
    ...
    function(warning_state, dropdown_state) {
        const overlay = document.getElementById('tur-warning-overlay');
        ...
        if (overlay) {
            ...
            if (warning_state && warning_state.show) {
                ...
                console.log('TUR Warning overlay shown');
            } else {
                ...
                console.log('TUR Warning overlay hidden');
            }
        }
    ...
    """,
    Output('tur-warning-overlay', 'data-status', allow_duplicate=True),
    [Input('tur-selection-warning-state', 'data'),
     Input('tur-dropdown-open-state', 'data')],
     prevent_initial_call=True
)
...
clientside_callback(
    ...
    """,
    Output('tur-virtual-dropdown-trigger', 'data-keyboard-handler'),
    Input('tur-virtual-dropdown-trigger', 'n_clicks'),
    prevent_initial_call=True
)
...
clientside_callback(
    ...
    function(n_clicks) {
        if (n_clicks > 0 && !window.turFilterDropdownClickListener) {
            setTimeout(function() {
                document.addEventListener('click', function(e) {
                    const trigger = document.getElementById('tur-add-filter-trigger-btn');
                    const menu = document.getElementById('tur-filter-dropdown-menu');
                    ...
                    if (trigger && menu && 
                        ...
                        if (menu.style.display === 'block') {
                            ...
                            if (window.dash_clientside && window.dash_clientside.set_props) {
                                try {
                                    window.dash_clientside.set_props('tur-filter-dropdown-open', {data: false});
                                } catch (error) {
                                    console.error('Error closing TUR filter dropdown:', error);
                                }
                            }
                        }
                    }
    ...
    """,
    Output('tur-add-filter-trigger-btn', 'data-click-listener'),
    Input('tur-add-filter-trigger-btn', 'n_clicks'),
    prevent_initial_call=True
)
...
# ==========================================
# CACHE MANAGEMENT - TURKIYE
# ==========================================

def clear_caches_turkiye():
    """Clear all Turkiye caches"""
    global _dropdown_cache_turkiye, _param_cache_turkiye, _comparison_cache_turkiye, _graph_cache_turkiye
    _dropdown_cache_turkiye.clear()
    _param_cache_turkiye.clear()
    _comparison_cache_turkiye.clear()
    _graph_cache_turkiye.clear()
    print("TUR: All caches cleared")
